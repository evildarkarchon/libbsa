...
<required_reading>
@J:/libbsa/.gemini/get-shit-done/references/domain-probes.md
@J:/libbsa/.gemini/get-shit-done/references/gate-prompts.md
@J:/libbsa/.gemini/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "J:/libbsa/.gemini/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@J:/libbsa/.gemini/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
