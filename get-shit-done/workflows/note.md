...
- **Global scope**: `J:/libbsa/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `J:/libbsa/notes/`)
...
2. Glob `J:/libbsa/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `J:/libbsa/notes/` — works in any directory
