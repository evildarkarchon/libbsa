...
std::vector<std::byte> build_rgba8_dds(std::uint32_t width, std::uint32_t height, std::uint8_t seed) {
  ...
  for (const auto value : {32U, 0x0000'0004U, dds_fourcc_dx10, 0U, 0U, 0U, 0U, 0U, dds_caps_texture, 0U, 0U, 0U, 0U}) {
    writer.u32(value);
  }
...
std::filesystem::path make_work_dir() {
  ...
  auto path = std::filesystem::temp_directory_path() / ("libbsa-benchmark-" + std::to_string(stamp));
  std::filesystem::create_directories(path);
  return path;
}

void add_disk_payloads(libbsa::tes4_bsa_writer& writer,
  ...
  for (std::size_t index = 0; index < entries.size(); ++index) {
    ...
    require_success(writer.add_file(entries[index].archive_path, source.string()), "add TES4 benchmark source");
  }
}

void add_disk_payloads(libbsa::ba2_gnrl_writer& writer,
                       const std::filesystem::path& root,
                       std::span<const named_payload> entries) {
  for (std::size_t index = 0; index < entries.size(); ++index) {
    const auto source = root / ("ba2-gnrl-source-" + std::to_string(index) + ".bin");
    write_binary_file(source, entries[index].bytes);
    require_success(writer.add_file(entries[index].archive_path, source.string()), "add BA2 GNRL benchmark source");
  }
}
...
void verify_archive_payloads(const std::filesystem::path& archive, std::span<const named_payload> entries) {
  auto reader = require_value(libbsa::archive_reader::open(archive.string()), "open benchmark archive");
  for (const auto& entry : entries) {
    auto extracted = require_value(reader.extract_bytes(entry.archive_path), "extract benchmark entry");
    if (extracted != entry.bytes) {
      throw std::runtime_error("benchmark extracted bytes mismatch for " + entry.archive_path);
    }
  }
}

benchmark_result run_tes4_bsa_pack_extract(const std::filesystem::path& work_dir, std::uint32_t worker_count) {
  const std::vector entries{
      named_payload{"Meshes/Benchmark/LargeA.nif", patterned_bytes(192U * 1024U, 0x11U)},
      named_payload{"Textures/Benchmark/LargeB.dds", patterned_bytes((160U * 1024U) + 37U, 0x31U)},
      named_payload{"Scripts/Benchmark/Small.pex", bytes_from_text("benchmark TES4 script payload")},
  };
...
benchmark_result run_ba2_gnrl_pack_extract(const std::filesystem::path& work_dir, std::uint32_t worker_count) {
  const std::vector entries{
      named_payload{"Meshes/Benchmark/GeneralA.nif", patterned_bytes(224U * 1024U, 0x41U)},
      named_payload{"Scripts/Benchmark/GeneralB.pex", patterned_bytes((144U * 1024U) + 19U, 0x61U)},
  };
  ...
  libbsa::write_execution_options execution;
  execution.worker_count = worker_count;
  require_success(writer.write_to(archive.string(), execution), "write BA2 GNRL benchmark archive");
  verify_archive_payloads(archive, entries);
  const auto elapsed = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
...
std::vector<benchmark_result> run_ba2_dx10_pack_extract(const std::filesystem::path& work_dir, std::uint32_t worker_count) {
  ...
  libbsa::write_execution_options execution;
  execution.worker_count = worker_count;
  require_success(writer.write_to(archive.string(), execution), "write BA2 DX10 benchmark archive");
  auto reader = require_value(libbsa::archive_reader::open(archive.string()), "open BA2 DX10 benchmark archive");
  ...
  if (found->texture->width != 256U || found->texture->height != 256U || found->texture->mip_count != 1U ||
      found->texture->dxgi_format != 28U) {
    throw std::runtime_error("BA2 DX10 benchmark texture metadata mismatch after reopen");
  }
  const auto extracted = require_value(reader.extract_bytes(archive_path), "extract BA2 DX10 benchmark entry");
...
benchmark_result run_bulk_extract(const std::filesystem::path& work_dir, std::uint32_t worker_count) {
  const std::vector entries{
      ...
      named_payload{"Docs/Benchmark/BulkC.txt", bytes_from_text("bulk extraction correctness payload")},
  };
  ...
  for (const auto& entry : entries) {
    require_success(writer.add_bytes(entry.archive_path, entry.bytes), "add bulk extraction source");
  }
  require_success(writer.write_to(archive.string()), "write bulk extraction source archive");

  auto reader = require_value(libbsa::archive_reader::open(archive.string()), "open bulk extraction benchmark archive");
  ...
  // This scenario specifically exercises archive_reader::extract_entries as the public bulk path.
  auto extracted = require_value(reader.extract_entries(requests, sink_factory, bulk_options), "bulk extract benchmark entries");
  const auto elapsed = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
  ...
  for (std::size_t index = 0; index < entries.size(); ++index) {
    if (!extracted[index].succeeded() || extracted[index].path != entries[index].archive_path) {
      throw std::runtime_error("bulk extraction benchmark result ordering or success mismatch");
    }
...
void write_json_report(const std::filesystem::path& path, std::span<const benchmark_result> results) {
  std::filesystem::create_directories(path.parent_path());
  ...
  for (std::size_t index = 0; index < results.size(); ++index) {
    ...
    out << "  {\"scenario\": \"" << json_escape(result.scenario) << "\", \"worker_count\": " << result.worker_count
        << ", \"elapsed_ms\": " << result.elapsed_ms << ", \"bytes_processed\": " << result.bytes_processed
...
void write_markdown_report(const std::filesystem::path& path, std::span<const benchmark_result> results) {
  std::filesystem::create_directories(path.parent_path());
  std::ofstream out{path, std::ios::binary | std::ios::trunc};
  if (!out) {
  ...
  out << "| scenario | worker_count | elapsed_ms | bytes_processed | correctness_passed |\n";
  out << "|----------|--------------|------------|-----------------|--------------------|\n";
  out << std::fixed << std::setprecision(3);
  for (const auto& result : results) {
    out << "| " << result.scenario << " | " << result.worker_count << " | " << result.elapsed_ms << " | "
        << result.bytes_processed << " | " << (result.correctness_passed ? "true" : "false") << " |\n";
  }
  if (!out) {
...
cli_options parse_cli(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (arg == "--output-json" && index + 1 < argc) {
      ...
      throw std::runtime_error("usage: libbsa_benchmarks --output-json <path> --output-markdown <path>");
    }
  }
...
/// Runs correctness-checked synthetic worker-count benchmark scenarios and writes reports.
