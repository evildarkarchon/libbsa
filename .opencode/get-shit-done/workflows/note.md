...
- **Global scope**: `J:/libbsa/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `J:/libbsa/.opencode/notes/`)
...
2. Glob `J:/libbsa/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (J:/libbsa/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `J:/libbsa/.opencode/notes/` — works in any directory
