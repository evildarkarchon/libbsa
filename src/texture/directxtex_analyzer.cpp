...
// The analyzer includes Windows-backed texture headers privately; NOMINMAX
// keeps those headers from rewriting standard-library min/max calls in this
// translation unit.
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
result<std::uint32_t> checked_u32(std::size_t value, const char* field_name) {
    if (value > (std::numeric_limits<std::uint32_t>::max)()) {
        return error{error_code::format_error,
                     std::string{field_name} + " exceeds public metadata limits"};
    }
...
/// Rejects DDS shapes the BA2 DX10 writer cannot round-trip without changing
/// resource dimension.
result<void> validate_writer_source_shape(const DirectX::TexMetadata& metadata) {
    if (metadata.dimension != DirectX::TEX_DIMENSION_TEXTURE2D) {
        return error{error_code::format_error,
                     "DDS source shape is unsupported by the BA2 DX10 writer"};
    }
    if (metadata.depth > 1U) {
        return error{error_code::format_error,
                     "DDS source depth is unsupported by the BA2 DX10 writer"};
    }
    if (metadata.IsCubemap() && (metadata.arraySize % 6U) != 0U) {
        return error{error_code::format_error,
                     "DDS cubemap source does not contain complete face groups"};
    }
    return {};
}
...
result<texture_metadata> translate_source_metadata(const DirectX::TexMetadata& metadata) {
    ...
    const auto logical_array_size =
        metadata.IsCubemap() ? metadata.arraySize / 6U : metadata.arraySize;
...
result<texture_metadata> analyze_dds_metadata(std::span<const std::byte> dds_bytes) {
    ...
    const HRESULT hr = DirectX::GetMetadataFromDDSMemory(dds_bytes.data(), dds_bytes.size(),
                                                         DirectX::DDS_FLAGS_NONE, metadata);
    if (hr < 0) {
        return error{error_code::format_error,
                     "DDS metadata could not be loaded by the texture analyzer"};
    }
...
result<dds_source_analysis> analyze_dds_source(std::span<const std::byte> dds_bytes) {
    ...
    const HRESULT hr = DirectX::LoadFromDDSMemory(dds_bytes.data(), dds_bytes.size(),
                                                  DirectX::DDS_FLAGS_NONE, &metadata, image);
    if (hr < 0) {
        return error{error_code::format_error,
                     "DDS source could not be loaded by the texture analyzer"};
    }
    ...
    if (!is_supported_writer_source_format(metadata.format)) {
        return error{error_code::format_error,
                     "DDS source format is unsupported by the BA2 DX10 writer"};
    }
    ...
    for (std::size_t index = 0; index < image_count; ++index) {
        ...
        // D-03 requires writer-owned snapshots: callers may delete or mutate the
        // source DDS after add. Copying here avoids storing DirectX-owned pointers
        // or borrowing caller file bytes across phases.
        analysis.image_payload_bytes.insert(analysis.image_payload_bytes.end(), copied.begin(),
                                            copied.end());
        analysis.subresources.push_back(
            dds_source_subresource{array_index, face_index, mip, std::move(copied)});
    }
