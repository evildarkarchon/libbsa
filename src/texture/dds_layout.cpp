...
bool validate_layout_shape(const dds_texture_layout& layout) noexcept {
    return layout.width != 0U && layout.height != 0U && layout.mip_count != 0U &&
           layout.array_size != 0U;
}
...
constexpr std::array locked_formats{
    ...
    dxgi_format_descriptor{96U, 4U, 4U, 16U},  // DXGI_FORMAT_BC6H_SF16.
...
const dxgi_format_descriptor* find_descriptor(std::uint32_t format) noexcept {
    const auto found =
        std::find_if(locked_formats.begin(), locked_formats.end(),
                     [format](const auto& descriptor) { return descriptor.format == format; });
...
std::uint64_t rounded_block_count(std::uint32_t dimension, std::uint32_t block_dimension) noexcept {
    ...
    // Promote before adding the block-rounding bias so hostile uint32 DDS
    // dimensions cannot wrap.
    return std::max<std::uint64_t>(1U, (static_cast<std::uint64_t>(dimension) +
                                        static_cast<std::uint64_t>(block_dimension) - 1U) /
                                           static_cast<std::uint64_t>(block_dimension));
}

result<std::uint64_t> described_mip_size(const dxgi_format_descriptor& descriptor,
                                         std::uint32_t width, std::uint32_t height) {
    // DirectX block-compressed DDS formats still allocate at least one 4x4 block
    // for tiny mips.
...
result<std::vector<planned_mip_range>> default_mip_ranges(const dds_texture_layout& layout) {
    ...
    // The reference-derived BA2 DX10 default emits a few large mips individually,
    // then groups the tail.
    while (mip + 1U < layout.mip_count && ranges.size() < 3U &&
           mip_dimension(layout.width, mip) >= 512U && mip_dimension(layout.height, mip) >= 512U) {
...
void append_chunks_for_ranges(std::vector<planned_texture_chunk>& chunks,
                              ...
                              std::uint32_t array_index, std::uint32_t face_index) {
    for (const auto& range : ranges) {
        chunks.push_back(planned_texture_chunk{array_index, face_index, range.start_mip,
                                               range.end_mip, range.raw_size});
    }
}
...
result<std::uint64_t> mip_size_for_format(const dds_texture_layout& layout, std::uint32_t mip) {
    ...
    return described_mip_size(*descriptor, mip_dimension(layout.width, mip),
                              mip_dimension(layout.height, mip));
}
...
result<std::vector<std::byte>> build_dds_dxt10_header(const dds_texture_layout& layout) {
    ...
    const std::uint32_t caps = dds_caps_texture |
                               (layout.mip_count > 1U ? dds_caps_complex | dds_caps_mipmap : 0U) |
                               (layout.is_cubemap ? dds_caps_complex : 0U);
    const std::uint32_t caps2 =
        layout.is_cubemap ? (dds_caps2_cubemap | dds_caps2_cubemap_all_faces) : 0U;

    // D-09 always emits the DXT10 extension, even for formats that could be
    // represented by legacy DDS pixel formats. This keeps extraction
    // deterministic and avoids per-format header branches.
    auto ok = write_u32(writer, dds_magic);
    if (!ok) {
        return ok.error();
    }
    for (const auto value : {dds_header_size, dds_flags, layout.height, layout.width,
                             dds_pitch_or_linear_size, dds_depth, layout.mip_count}) {
    ...
    for (const auto value : {dds_pixel_format_size, dds_pixel_format_fourcc_flag, dds_fourcc_dx10,
                             0U, 0U, 0U, 0U, 0U, caps, caps2, 0U, 0U, 0U}) {
        ok = write_u32(writer, value);
        if (!ok) {
            return ok.error();
        }
    ...
    // D-18 defaults unclear DXT10 fields conservatively: texture2D resource
    // dimension, cube misc flag only for cubemaps, and alpha metadata left at
    // zero.
...
result<std::vector<planned_texture_chunk>> plan_dx10_chunks(const dds_texture_layout& layout,
    ...
    auto ranges = max_decoded_chunk_bytes == 0U
                      ? default_mip_ranges(layout)
    ...
    chunks.reserve(static_cast<std::size_t>(layout.array_size) * faces_per_array *
                   ranges.value().size());
...
result<std::vector<logical_texture_segment>> validate_and_order_chunks(
    ...
    // D-12: normal extraction must not ask DirectXTex whether these bytes are
    // acceptable; this metadata-only pass fails closed before payload reads and
    // leaves DirectXTex to tests/analyzers. D-15: BA2 texture chunks are accepted
    // only in computed DDS order: array slices ascend first, then
    // reference-backed cubemap face order +X, -X, +Y, -Y, +Z, -Z, then mip ranges
    // ascend.
    const std::uint32_t faces_per_array = layout.is_cubemap ? 6U : 1U;
    ...
    for (std::size_t source_chunk_index = 0; source_chunk_index < chunks.size();
         ++source_chunk_index) {
        ...
        // D-15/DDS-07 permits repeated mip ranges across different array slices or
        // cubemap faces, but within one face/slice a repeated or earlier range is a
        // duplicate/overlap instead of a new face.
        if (chunk.start_mip != expected_start_mip) {
            return format_error(
                "DDS layout chunk sequence has a mip gap, duplicate, "
                "or contradicts BA2 order");
        }
        ...
        segments.push_back(logical_texture_segment{array_index, face_index, chunk.start_mip,
