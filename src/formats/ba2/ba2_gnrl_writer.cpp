...
result<void> ba2_gnrl_writer::add_file(std::string_view archive_path, std::string_view host_path,
...
result<void> ba2_gnrl_writer::add_file(std::string_view archive_path, std::string_view host_path,
    ...
    if (host_path.empty()) {
        return error{error_code::invalid_argument,
                     "BA2 GNRL disk source host path must not be empty"};
    }
...
result<void> ba2_gnrl_writer::write_to(std::string_view host_path,
    ...
    return formats::ba2::write_ba2_gnrl_archive(state_->target, state_->options, state_->entries,
                                                host_path, execution.worker_count);
}
