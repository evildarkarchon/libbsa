    ...
    for (auto& entry : entries) {
        for (auto& chunk : entry.chunks) {
            if (deduplicate_payloads) {
                // D-20 requires DX10 dedupe to prove both byte identity and chunk
                // metadata identity. Two chunks only share storage when the final
                // stored bytes, raw size, packed size, and explicit compression route
                // all match; texture dimensions and mip identity remain separate
                // records.
                dedupe_key key{chunk.stored_payload, chunk.raw_size, chunk.packed_size,
                               chunk.compression};
