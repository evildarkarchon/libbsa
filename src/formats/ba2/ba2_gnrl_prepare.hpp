...
struct ba2_gnrl_prepared_entry {
    ...
    // Dedupe narrowing uses a deliberately named final-stored fingerprint so
    // policy checks can distinguish it from caller-source hashes; exact equality
    // in layout remains the authority for shared offsets.
