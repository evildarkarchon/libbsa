...
namespace libbsa::formats::ba2
{
  ...
  result<ba2_dx10_header_fields> read_ba2_dx10_header(detail::binary_reader &reader)
  {
    ...
    return ba2_dx10_header_fields{magic.value(),
                                  version.value(),
                                  subtype.value(),
                                  file_count.value(),
                                  file_table_offset.value(),
                                  ba2};
  }
                                                             ...
                                                             std::uint64_t file_table_offset)
  {
    ...
    for (std::uint32_t index = 0; index < file_count; ++index)
    {
      ...
      if (!detail::add_fits_u64(aggregate_chunk_count, chunk_count.value(), next_aggregate_chunk_count) ||
      ...
      ba2_dx10_record record{name_hash.value(),
                             extension,
      ...
      for (std::uint8_t chunk_index = 0; chunk_index < chunk_count.value(); ++chunk_index)
      {
        ...
        record.chunks.push_back(ba2_dx10_chunk_record{offset.value(),
                                                      packed_size.value(),
                                                      raw_size.value(),
                                                      start_mip.value(),
                                                      end_mip.value()});
