...
struct ba2_dx10_writer::state {
    ...
    /// Removes the writer-owned snapshot directory without changing the primary
    /// result that triggered cleanup.
    void cleanup_snapshot_dir() noexcept {
        std::error_code fs_error;
        // D-10 keeps cleanup best-effort so validation, write, or publish failures
    ...
    ~state() {
        // Destruction remains a safety net for abandoned writers that never reached
        // write_to cleanup.
        cleanup_snapshot_dir();
    }
};

ba2_dx10_writer::ba2_dx10_writer(ba2_dx10_target target)
    : ba2_dx10_writer(target, ba2_dx10_writer_options{}) {}
...
result<void> ba2_dx10_writer::add_file(std::string_view archive_path,
    ...
    if (dds_host_path.empty()) {
        return error{error_code::invalid_argument,
                     "BA2 DX10 DDS source host path must not be empty"};
    }
    ...
    // D-17 keeps add-time ownership while avoiding a long-lived full DDS byte
    // vector in writer state.
...
result<void> ba2_dx10_writer::write_to(std::string_view host_path,
    ...
    // D-03 keeps one-shot semantics local to BA2 DX10 because snapshot files are
    // discarded after an attempt.
...
result<void> write_ba2_dx10_archive(ba2_dx10_target target, const ba2_dx10_writer_options& options,
    ...
    return detail::publish_writer_output(
        ...
        [&](const detail::finalization_workspace& workspace) -> result<void> {
            auto prepared =
                ba2_dx10_prepare_entries(profile.value(), options, entries, worker_count);
            if (!prepared) {
