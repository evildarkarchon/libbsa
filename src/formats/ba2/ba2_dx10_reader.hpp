...
/// Returns stable BA2 DX10 entry metadata copies sorted by canonical archive
/// path.
result<std::vector<entry_metadata>> ba2_dx10_entries(std::span<const entry_metadata> entries);

/// Looks up one BA2 DX10 entry using public archive-path normalization
/// semantics.
result<std::optional<entry_metadata>> find_ba2_dx10_entry(std::span<const entry_metadata> entries,
                                                          std::string_view path);

/// Reports BA2 DX10 entry presence using the same normalization and errors as
/// find.
