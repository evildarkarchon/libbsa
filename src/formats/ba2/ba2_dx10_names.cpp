...
namespace libbsa::formats::ba2
{
                                                               ...
                                                               std::uint32_t file_count)
  {
    ...
    for (std::uint32_t index = 0; index < file_count; ++index)
    {
      ...
      auto length_bytes = detail::read_file_bytes_at(input, cursor, 2U, "BA2 DX10 filename length");
      if (!length_bytes)
      {
      ...
      auto name_bytes = detail::read_file_bytes_at(input, cursor, length.value(), "BA2 DX10 filename bytes");
      if (!name_bytes)
      {
