...
result<ba2_dx10_prepared_chunk> ba2_dx10_assemble_chunk(ba2_dx10_target target,
  ...
  // Snapshot files are the source of truth after add_file, and bounded chunk streaming catches
  // missing/truncated temp data before publish can replace caller-visible output.
...
result<ba2_dx10_prepared_entry> ba2_dx10_assemble_planned_entry(ba2_dx10_target target,
                                                               const ba2_dx10_writer_options& options,
  ...
  // Indexed work preserves planned texture order even when worker_count enables parallel assembly.
