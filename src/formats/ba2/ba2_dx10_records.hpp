...
namespace libbsa::formats::ba2
{
  ...
  /// Raw BA2 DX10 texture chunk record exactly as stored in the texture record table.
  ...
  /// Reads the fixed BA2 DX10 header without consuming texture records or filename-table bytes.
  [[nodiscard]] result<ba2_dx10_header_fields> read_ba2_dx10_header(detail::binary_reader &reader);

  /// Reads the fixed-width BA2 DX10 texture record and chunk tables without materializing public entries.
  [[nodiscard]] result<std::vector<ba2_dx10_record>> read_ba2_dx10_records(detail::binary_reader &reader,
                                                                           std::uint32_t file_count,
                                                                           std::uint64_t file_table_offset);
