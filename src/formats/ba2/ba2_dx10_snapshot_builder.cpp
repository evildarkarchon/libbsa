...
// BCrypt depends on Windows base types; keep both headers private and prevent
// min/max macros.
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
/// Returns true for DDS DXGI formats accepted by the Starfield DX10 profile but
/// not Fallout 4.
...
constexpr detail::host_file_context ba2_dx10_dds_source_context{
    "BA2 DX10 writer failed to open DDS source", "BA2 DX10 writer failed to inspect DDS source",
    "BA2 DX10 writer failed while reading DDS source",
    "BA2 DX10 DDS source changed during analysis", "BA2 DX10 DDS source"};
...
/// Generates a 128-bit lowercase hex suffix using the Windows system-preferred
/// RNG. Returns `io_error` if Windows cannot provide fresh cryptographic
/// randomness for a candidate.
result<std::string> make_snapshot_random_suffix() {
    std::array<std::byte, snapshot_random_suffix_bytes> random_bytes{};
    const auto status =
        BCryptGenRandom(nullptr, reinterpret_cast<PUCHAR>(random_bytes.data()),
                        static_cast<ULONG>(random_bytes.size()), BCRYPT_USE_SYSTEM_PREFERRED_RNG);
    if (status < 0) {
        return error{error_code::io_error,
                     "BA2 DX10 writer failed to generate snapshot temp directory name"};
    }
...
result<std::filesystem::path> make_unique_snapshot_directory() {
    ...
    for (std::uint32_t attempt = 0; attempt < 1024U; ++attempt) {
        ...
        fs_error.clear();
        // Directory creation remains the atomic reservation boundary; existing
        // paths are collisions.
        if (std::filesystem::create_directory(candidate, fs_error)) {
            return candidate;
        }
        if (fs_error) {
            return error{error_code::io_error,
                         "BA2 DX10 writer failed to reserve snapshot temp directory"};
        }
    }
    return error{error_code::io_error, "BA2 DX10 writer exhausted snapshot temp directory names"};
}

result<void> write_snapshot_file(const std::filesystem::path& snapshot_path,
    ...
    if (!output) {
        return error{error_code::io_error,
                     "BA2 DX10 writer failed while writing snapshot temp file"};
    }
    return {};
}
        ...
        case ba2_dx10_target::fallout4:
            if (is_starfield_only_dx10_format(dxgi_format)) {
                return error{error_code::format_error,
                             "BA2 DX10 Fallout 4 target does not support BC6, SRGB, or "
                             "SNORM DDS formats"};
            }
            return {};
...
result<ba2_dx10_writer_entry> ba2_dx10_build_writer_entry_snapshot(
    ...
    auto target_format =
        ba2_dx10_validate_texture_format_for_target(target, source.value().metadata.dxgi_format);
    if (!target_format) {
    ...
    for (std::size_t index = 0; index < source.value().subresources.size(); ++index) {
        ...
        auto snapshot_path = snapshot_dir / ("entry-" + std::to_string(entry_index) +
                                             "-subresource-" + std::to_string(index) + ".bin");
        ...
        entry.subresources.push_back(ba2_dx10_subresource_snapshot{
            subresource.array_index, subresource.face_index, subresource.mip,
            subresource.bytes.size(), std::move(snapshot_path)});
    }
    return entry;
}
