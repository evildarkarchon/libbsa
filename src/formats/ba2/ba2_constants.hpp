...
// TES5Edit's BA2 readers and writers require the BAADF00D sentinel after each
...
// Starfield BA2 v3 routes CompressionMethod 0 through deflate and method 3
// through raw LZ4 blocks.
...
// TES5Edit writes and reads a fixed 24-byte DX10 chunk header; other widths
// make record parsing drift.
inline constexpr std::uint16_t ba2_dx10_chunk_header_size = 24U;

// BA2 DX10 stores cubemap state as raw marker values rather than a boolean
// field.
inline constexpr std::uint16_t ba2_dx10_non_cubemap_raw = 2048U;
inline constexpr std::uint16_t ba2_dx10_cubemap_raw = 2049U;

// Reference-derived writer-owned texture byte; not public until stronger
// compatibility evidence requires it.
