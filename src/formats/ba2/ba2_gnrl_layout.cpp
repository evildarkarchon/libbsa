...
ba2_gnrl_final_stored_dedupe_key make_ba2_gnrl_final_stored_dedupe_key(
    std::uint64_t final_stored_dedupe_hash, std::uint32_t stored_size) noexcept {
...
constexpr detail::host_file_context ba2_gnrl_dedupe_source_context{
    ...
    "BA2 GNRL disk source changed during dedupe preparation", "BA2 GNRL disk source"};
...
result<bool> ba2_gnrl_payloads_equal(const ba2_gnrl_prepared_entry& lhs,
    ...
    if (lhs.stream_from_disk && rhs.stream_from_disk) {
        return compare_disk_payloads(lhs.resolved_source_path, rhs.resolved_source_path,
                                     lhs.raw_size);
    }
...
result<void> ba2_gnrl_assign_payload_offsets(std::span<ba2_gnrl_prepared_entry> entries,
    ...
    std::map<ba2_gnrl_final_stored_dedupe_key, std::vector<payload_assignment>>
        deduplicated_payloads;
    for (std::size_t index = 0; index < entries.size(); ++index) {
        auto& entry = entries[index];
        auto stored_size =
            checked_u32(entry.stream_from_disk ? entry.raw_size : entry.stored_payload.size(),
                        "BA2 GNRL stored payload size");
        if (!stored_size) {
            return stored_size.error();
        }
        if (deduplicate_payloads) {
            // D-23 requires dedupe after raw-vs-compressed routing, so this key is
            // the exact byte span the writer would store in the BA2 payload area
            // rather than the caller's source bytes.
            const auto final_stored_dedupe_hash = entry.final_stored_dedupe_hash;
            const auto identity = make_ba2_gnrl_final_stored_dedupe_key(final_stored_dedupe_hash,
                                                                        stored_size.value());
        ...
        if (deduplicate_payloads) {
            const auto identity = make_ba2_gnrl_final_stored_dedupe_key(
                entry.final_stored_dedupe_hash, stored_size.value());
...
result<bool> compare_disk_payload_to_bytes(const detail::host_file_path& host_path,
    ...
    auto compared = detail::for_each_host_file_chunk(
        host_path, expected.size(), ba2_gnrl_dedupe_source_context,
...
result<bool> compare_disk_payloads(const detail::host_file_path& lhs_path,
    ...
    while (remaining > 0U) {
        ...
        if (!std::equal(lhs_scratch.begin(),
                        lhs_scratch.begin() + static_cast<std::ptrdiff_t>(requested),
                        rhs_scratch.begin())) {
            return false;
        }
        remaining -= requested;
