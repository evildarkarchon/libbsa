...
result<void> stream_raw_payload(const detail::host_file_path& host_path,
                                const entry_metadata& entry, payload_sink& sink) {
    if (entry.raw_size != entry.stored_size) {
        return error{error_code::format_error,
                     "BA2 GNRL raw payload size does not match stored size"};
    }
...
result<void> extract_compressed_payload(const detail::host_file_path& host_path,
    ...
    // Corrupt BA2 compressed payloads are malformed archive bytes, so preserve
    // the codec's stable format_error result instead of attempting partial
    // extraction.
    return detail::decompress_payload_exact_to_sink(
        method.value(), input.value(), entry.payload_offset, entry.stored_size, entry.raw_size,
        sink, extraction_chunk_size, "BA2 GNRL compressed payload");
}
...
result<void> extract_ba2_gnrl_payload(const detail::host_file_path& host_path,
    ...
    if (entry.has_embedded_name || entry.embedded_name_prefix_size != 0U) {
        return error{error_code::format_error,
                     "BA2 GNRL entries must not carry embedded-name prefixes"};
    }
