...
// Reader and writer paths depend on name tables being present to expose stable public archive paths.
...
// This archive flag sets the default; the per-file size flag below XORs that default for one entry.
inline constexpr std::uint32_t tes4_bsa_archive_compress_by_default = 0x0004U;

// Embedded names are payload prefixes and are intentionally suppressed for Oblivion-compatible archives.
...
// The high size bit is not part of the stored payload length; it toggles compression from the archive default.
