...
result<void> tes3_bsa_writer::add_file(std::string_view archive_path, std::string_view host_path) {
    auto validated_host_path =
        formats::bsa::tes3_validate_host_path(host_path, "TES3 BSA disk source host path");
...
result<void> tes3_bsa_writer::add_bytes(std::string_view archive_path,
...
result<void> tes3_bsa_writer::write_to(std::string_view host_path,
...
result<void> write_tes3_bsa_archive(const tes3_bsa_writer_options& options,
    ...
    auto validated_host_path =
        tes3_validate_host_path(output_host_path, "TES3 BSA output host path");
    if (!validated_host_path) {
        return validated_host_path.error();
    }
    ...
    return detail::publish_writer_output(
        ...
        [&](const detail::finalization_workspace& workspace) -> result<void> {
            // The workspace must exist before D-16 preparation inspects a
            // path-backed source, and it must outlive serialization and publish.
