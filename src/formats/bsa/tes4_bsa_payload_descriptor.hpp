...
/// Private TES4 payload facts derived from a raw file record before public
...
/// Reads prefix-dependent TES4 payload metadata through the parser's
...
result<tes4_bsa_payload_descriptor> make_tes4_bsa_payload_descriptor(
    ...
    if (profile.reader_has_embedded_names(header.archive_flags)) {
        ...
        const auto length =
            static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes.value()[0]));
        embedded_prefix = length + 1U;
        if (embedded_prefix > stored_size.value() ||
            !detail::span_fits(record.offset, embedded_prefix, archive_size)) {
            return error{error_code::format_error,
    ...
    return tes4_bsa_payload_descriptor{record.offset, stored_size.value(), embedded_prefix,
                                       raw_size, compression};
}
