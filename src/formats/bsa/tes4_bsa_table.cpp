...
namespace libbsa::formats::bsa
{
  namespace
  {
    ...
    result<std::vector<tes4_bsa_folder_record>> read_folder_records(detail::binary_reader &reader,
    ...
    result<void> validate_tables(detail::binary_reader &reader, const tes4_bsa_header_fields &header,
                                 std::span<const tes4_bsa_folder_record> folders, std::size_t archive_size)
    {
      std::size_t file_records_seen = 0;
    ...
    result<std::vector<tes4_bsa_folder_block>> read_folder_blocks(detail::binary_reader &reader,
                                                                  const tes4_bsa_header_fields &header,
                                                                  std::span<const tes4_bsa_folder_record> folders)
    {
      ...
      for (const auto &folder : folders)
      {
        ...
        for (std::uint32_t index = 0; index < folder.file_count; ++index)
        {
          ...
          file_records.push_back(tes4_bsa_file_record{hash.value(), size_flags.value(), offset.value()});
        }
        file_records_seen += folder.file_count;
        blocks.push_back(tes4_bsa_folder_block{std::move(folder_name.value()), std::move(file_records)});
      }
                                                     ...
                                                     detected_bsa_format detected)
  {
    ...
    auto table_size = tes4_bsa_metadata_table_size(header.value(), folder_record_size, archive_size);
    if (!table_size)
    {
    ...
    auto skipped_header_and_records = table_validator.skip(tes4_bsa_header_size + folder_records_size);
