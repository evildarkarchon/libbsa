...
namespace libbsa::formats::bsa
{
                                                       ...
                                                       detected_bsa_format detected)
  {
    ...
    auto header_bytes = read_file_bytes_at(input.value(), 0U, fixed_header_size, "TES3 BSA fixed header");
    if (!header_bytes)
    {
    ...
    auto table_bytes = read_file_bytes_at(input.value(), 0U, table_size.value(), "TES3 BSA metadata table");
    if (!table_bytes)
    {
