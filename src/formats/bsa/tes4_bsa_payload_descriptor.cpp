...
entry_compression tes4_bsa_compression_for(const tes4_bsa_header_fields& header,
    ...
    const bool default_compressed =
        (header.archive_flags & tes4_bsa_archive_compress_by_default) != 0U;
    ...
    return header.version == tes4_bsa_skyrim_se_version ? entry_compression::lz4_frame
                                                        : entry_compression::deflate;
}

bool tes4_bsa_has_embedded_names(const tes4_bsa_header_fields& header) noexcept {
    return header.version != tes4_bsa_oblivion_version &&
