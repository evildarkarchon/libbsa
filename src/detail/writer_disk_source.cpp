...
result<std::size_t> checked_buffer_size(std::uint64_t size, const writer_disk_source_context& context) {
...
result<void> validate_expected_source_size(std::string_view host_path,
                                           std::uint64_t expected_size,
                                           const writer_disk_source_context& context) {
  auto actual_size = inspect_disk_source_size(host_path, context);
  if (!actual_size) {
...
result<void> read_exact_bytes(std::ifstream& input,
...
result<void> reject_appended_source_byte(std::ifstream& input, const writer_disk_source_context& context) {
  char extra = '\0';
  if (input.get(extra)) {
    return io_error(context.changed_error);
  }
...
result<std::vector<std::byte>> read_disk_source_exact(std::string_view host_path,
  ...
  auto stable = reject_appended_source_byte(input.value(), context);
  if (!stable) {
    return stable.error();
  }
...
result<std::vector<std::byte>> read_disk_source_exact(std::string_view host_path,
                                                      const writer_disk_source_context& context) {
  auto size = inspect_disk_source_size(host_path, context);
  if (!size) {
    return size.error();
  }
  return read_disk_source_exact(host_path, size.value(), context);
}
