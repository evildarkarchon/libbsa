...
/// Writes an archive to an isolated temporary path, then publishes that
/// completed file to the final host path.
///
/// The callback receives the reserved temporary file path and must return
/// `result<void>` after writing a complete archive there. The helper owns
/// existence/race checks, atomic/no-replace publication, and best-effort
/// cleanup.
...
result<void> publish_writer_output(const std::filesystem::path& output_path,
    ...
    auto published = publish_completed_writer_output(temp_path, output_path, overwrite_existing,
                                                     diagnostic_prefix);
    if (!published) {
        cleanup_writer_publish_directory(temp_dir.value());
