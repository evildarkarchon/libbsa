...
result<void> validate_metadata_count(std::uint64_t count, std::uint64_t limit,
    ...
    if (count > limit) {
        return error{error_code::format_error,
                     std::string{description} + " exceeds libbsa metadata count limit"};
    }
    return {};
}
...
result<std::vector<std::byte>> read_file_bytes_at(std::ifstream& input, std::uint64_t offset,
    ...
    if (!input) {
        return error{error_code::io_error,
                     std::string{"failed to seek while reading "} + std::string{description}};
    }
...
result<std::string> archive_string_from_bytes(std::span<const std::byte> bytes,
    ...
    if (bytes.size() > result.max_size()) {
        return error{error_code::format_error,
                     std::string{description} + " exceeds platform string limits"};
    }
