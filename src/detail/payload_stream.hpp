...
///
/// Partial sink acceptance is reported as `io_error` so callers never observe
/// an ambiguous partial-success extraction or packing operation.
...
/// Converts an archive-declared payload size to a single-vector allocation
/// size.
result<std::size_t> checked_materialized_payload_size(std::uint64_t value,
                                                      std::string_view description);

/// Validates that an archive-controlled byte range is representable by host
/// streams.
...
/// Streams an archive byte range to a sink without allocating the whole
/// payload.
