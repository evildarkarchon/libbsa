...
struct compressor_deleter {
  void operator()(libdeflate_compressor* compressor) const noexcept { libdeflate_free_compressor(compressor); }
};

struct decompressor_deleter {
  void operator()(libdeflate_decompressor* decompressor) const noexcept { libdeflate_free_decompressor(decompressor); }
};
...
libbsa::error codec_error() {
  return {libbsa::error_code::format_error, "raw deflate payload could not be decoded to the expected size"};
}

} // namespace

result<std::vector<std::byte>> compress_deflate(std::span<const std::byte> input, int compression_level) {
