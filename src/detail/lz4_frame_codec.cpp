...
result<void> decompress_lz4_frame_exact_to_sink(std::ifstream& input,
  ...
  auto input_buffer_result = make_byte_vector(chunk_size, std::string{description} + " compressed scratch buffer");
  if (!input_buffer_result) {
    return input_buffer_result.error();
  }
  auto output_buffer_result = make_byte_vector(chunk_size, std::string{description} + " decoded scratch buffer");
  ...
  if (!input) {
    return libbsa::error{libbsa::error_code::io_error, std::string{"failed to seek to "} + std::string{description}};
  }
  ...
  while (compressed_remaining != 0U) {
    const auto read_size = static_cast<std::size_t>(std::min<std::uint64_t>(compressed_remaining, input_buffer.size()));
    input.read(reinterpret_cast<char*>(input_buffer.data()), static_cast<std::streamsize>(read_size));
    if (input.bad()) {
      return libbsa::error{libbsa::error_code::io_error, std::string{"failed while reading "} + std::string{description}};
    }
    if (static_cast<std::size_t>(input.gcount()) != read_size) {
      return libbsa::error{libbsa::error_code::format_error, std::string{description} + " span is outside the archive"};
    }
    ...
    while (source_offset < read_size) {
      ...
      const auto output_capacity = static_cast<std::size_t>(std::min<std::uint64_t>(remaining_output, output_buffer.size()));
      ...
      source_offset += source_size;
      if (output_size != 0U) {
        auto written = write_payload_exact(sink, std::span<const std::byte>{output_buffer.data(), output_size}, description);
        if (!written) {
      ...
      if (status == 0U) {
        if (decoded_total != expected_size || source_offset != read_size || compressed_remaining != 0U) {
          return frame_error();
        }
        return {};
      }
