...
std::uint8_t lower_byte(char value) noexcept {
    ...
    if (byte >= static_cast<unsigned char>('A') && byte <= static_cast<unsigned char>('Z')) {
        return static_cast<std::uint8_t>(byte + static_cast<unsigned char>('a') -
...
std::uint64_t hash_tes3(std::string_view archive_path) {
    // TES5Edit/Core/wbBSArchive.pas CreateHashTES3 halves the byte string,
    // lowercases ASCII only via LowerByte, then XORs shifted bytes with a rotate
    // in the low half.
    const auto half = archive_path.size() >> 1U;
    std::uint32_t sum = 0;
    std::uint32_t offset = 0;
    for (std::size_t index = 0; index < half; ++index) {
        const auto temp = static_cast<std::uint32_t>(lower_byte(archive_path[index]))
                          << (offset & 31U);
        sum ^= temp;
        offset += 8U;
    }
...
std::uint32_t tes3_hash_low32(std::uint64_t hash) noexcept {
    return static_cast<std::uint32_t>(hash & 0xFFFF'FFFFULL);
}

std::uint32_t tes3_hash_high32(std::uint64_t hash) noexcept {
    return static_cast<std::uint32_t>(hash >> 32U);
}
...
std::uint32_t hash_fo4(std::string_view archive_path) {
    // TES5Edit/Core/wbBSArchive.pas CreateHashFO4 uses CRC32 with initial value
    // 0, ASCII LowerByte folding, skips bytes >127, and treats '/' as '\\'.
