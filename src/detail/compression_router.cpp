...
result<void> decompress_payload_exact_to_sink(compression_method method, std::ifstream& input,
    ...
    switch (method) {
        case compression_method::none:
            ...
            return stream_payload_range(input, compressed_offset, compressed_size, sink, chunk_size,
                                        description);
        case compression_method::lz4_frame:
        ...
        case compression_method::lz4_block: {
            ...
            auto stored =
                read_payload_bytes_at(input, compressed_offset, compressed_size, description);
            if (!stored) {
                return stored.error();
            }
