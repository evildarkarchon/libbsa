...
/// Decompresses an LZ4 frame archive range and writes exactly `expected_size`
/// bytes to `sink`.
///
/// Input and output scratch buffers are bounded by `chunk_size`. Malformed
/// frame data or exact-size mismatches return `format_error`; a caller sink may
/// have received partial decoded output before a later frame error is detected.
