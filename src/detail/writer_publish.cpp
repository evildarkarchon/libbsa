...
result<bool> path_exists_noexcept(const std::filesystem::path& path,
    ...
    if (fs_error) {
        return error{error_code::io_error,
                     prefixed_message(diagnostic_prefix, "failed to inspect output host path")};
    }
    return exists;
}

result<bool> path_is_reparse_point_noexcept(const std::filesystem::path& path,
                                            std::string_view diagnostic_prefix) {
#if defined(_WIN32)
    ...
    // Symlinks to files can still look regular through std::filesystem, so check
    // the Windows path itself.
...
result<void> validate_writer_output_path_before_publish(const std::filesystem::path& output_path,
    ...
    if (!overwrite_existing) {
        if (output_exists.value()) {
            return error{error_code::io_error,
                         prefixed_message(diagnostic_prefix, "output host path already exists")};
        }
        return {};
    }
    ...
    if (output_is_reparse_point.value()) {
        return error{error_code::io_error,
                     prefixed_message(diagnostic_prefix,
                                      "refuses to replace reparse-point output host path")};
    }
    ...
    if (output_exists.value()) {
        ...
        if (fs_error || !is_regular) {
            return error{error_code::io_error,
                         prefixed_message(diagnostic_prefix,
                                          "refuses to replace non-regular output host path")};
        }
    }
...
result<void> publish_completed_writer_output(const std::filesystem::path& temp_path,
                                             ...
                                             std::string_view diagnostic_prefix) {
    if (overwrite_existing) {
        auto output_is_reparse_point =
            path_is_reparse_point_noexcept(output_path, diagnostic_prefix);
        if (!output_is_reparse_point) {
            return output_is_reparse_point.error();
        }
        if (output_is_reparse_point.value()) {
        ...
        if (!published) {
            return error{error_code::io_error,
                         prefixed_message(diagnostic_prefix, "failed to publish output host path")};
        }
        return {};
    }
    ...
    if (!published) {
        return error{error_code::io_error,
                     prefixed_message(diagnostic_prefix,
                                      "failed to publish output host path without overwrite")};
    }
    return {};
}
