...
result<std::uint16_t> binary_reader::read_u16_le() {
    ...
    const auto value = static_cast<std::uint16_t>(
        ...
        static_cast<std::uint16_t>(
            static_cast<std::uint16_t>(static_cast<std::uint8_t>(bytes.value()[1])) << 8U));
    return value;
}
...
result<std::uint32_t> binary_reader::read_u32_le() {
    ...
    for (std::size_t index = 0; index < 4; ++index) {
        value |= static_cast<std::uint32_t>(static_cast<std::uint8_t>(bytes.value()[index]))
                 << (index * 8U);
    }
    return value;
}
...
result<std::uint64_t> binary_reader::read_u64_le() {
    ...
    for (std::size_t index = 0; index < 8; ++index) {
        value |= static_cast<std::uint64_t>(static_cast<std::uint8_t>(bytes.value()[index]))
                 << (index * 8U);
    }
    return value;
}
