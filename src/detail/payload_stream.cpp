...
result<std::size_t> checked_payload_size(std::uint64_t value, std::string_view description) {
  if (value > static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max())) {
    return libbsa::error{libbsa::error_code::format_error, std::string{description} + " exceeds platform limits"};
  }
...
result<std::vector<std::byte>> read_payload_bytes_at(std::ifstream& input,
  ...
  if (static_cast<std::size_t>(input.gcount()) != bytes.value().size()) {
    return libbsa::error{libbsa::error_code::format_error, std::string{description} + " is truncated"};
  }
...
result<void> stream_payload_range(std::ifstream& input,
  ...
  while (remaining != 0U) {
    ...
    auto written = write_payload_exact(sink, std::span<const std::byte>{buffer.data(), count}, description);
    if (!written) {
      return written.error();
    }
    remaining -= count;
