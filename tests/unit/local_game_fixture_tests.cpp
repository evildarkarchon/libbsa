...
namespace
{
  ...
  {
  public:
    /// Hashes bytes as they stream out of libbsa without storing local corpus payloads.
  ...
  void require_metadata_matches(const nlohmann::json &expected, const libbsa::archive_metadata &metadata)
  {
    ...
    {
      CHECK(metadata.variant == archive_variant_from_string(expected.at("variant").get<std::string>()));
    }
  ...
  void require_entry_metadata_matches(const nlohmann::json &expected, const libbsa::entry_metadata &entry)
  {
    ...
    {
      CHECK(entry.compression == entry_compression_from_string(expected.at("compression").get<std::string>()));
    }
    ...
    {
      CHECK(entry.embedded_name_prefix_size == u32_from_json(expected.at("embedded_name_prefix_size")));
    }
  }

  void require_entry_bytes_match(const libbsa::archive_reader &reader,
          ...
          "[requires-game-fixture][unit][compat]")
{
  ...
  {
    SKIP("Set LIBBSA_BSARCHPRO_EXPECTED to a BSArchPro-derived manifest, or place bsarchpro_expected.json under LIBBSA_GAME_FIXTURES.");
  }
  ...
  for (const auto &test_case : manifest.at("cases"))
  {
    ...
    auto opened = libbsa::archive_reader::open(archive_path_from_manifest(*manifest_path, test_case).string());
