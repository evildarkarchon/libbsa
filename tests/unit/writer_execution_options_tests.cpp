...
std::filesystem::path generated_dx10_source_path() {
    return std::filesystem::path{LIBBSA_SOURCE_DIR} / "tests" / "fixtures" / "generated" /
           "source" / "ba2_dx10_multi_mip_bc7_unorm.dds";
}
...
void require_entry_bytes(const std::filesystem::path& archive_path, std::string_view entry_path,
...
void require_matching_archive_entries(const std::filesystem::path& lhs,
...
TEST_CASE("writer_execution_options defaults to serial worker count",
...
TEST_CASE("writer_execution_options rejects zero worker count before creating output",
    ...
    SECTION("TES3 writer") {
        ...
        REQUIRE(
            writer.add_file("Meshes/Missing.NIF", output_path("missing-tes3-source.nif").string())
                .has_value());
    ...
    SECTION("TES4-family writer") {
        ...
        REQUIRE(
            writer.add_file("Meshes/Missing.NIF", output_path("missing-tes4-source.nif").string())
                .has_value());
    ...
    SECTION("BA2 GNRL writer") {
        ...
        REQUIRE(
            writer.add_file("Meshes/Missing.nif", output_path("missing-gnrl-source.nif").string())
                .has_value());
...
TEST_CASE(
    "writer_execution_options rejects unsupported large worker counts "
    ...
    SECTION("TES4-family writer") {
        ...
        REQUIRE(writer.add_bytes("Meshes/LargeWorker.NIF", bytes_from_text("tes4 payload"))
                    .has_value());
    ...
    SECTION("BA2 GNRL writer") {
        ...
        REQUIRE(writer.add_bytes("Meshes/LargeWorker.nif", bytes_from_text("ba2 gnrl payload"))
                    .has_value());
...
TEST_CASE(
    "writer_execution_options worker count one preserves serial writer "
    "behavior",
    "[unit][writer_execution_options]") {
    libbsa::write_execution_options execution;
    ...
    SECTION("BA2 DX10 writer") {
        ...
        REQUIRE(serial_writer
                    .add_file("textures/serial/texture.dds", generated_dx10_source_path().string())
                    .has_value());
        REQUIRE(execution_writer
                    .add_file("textures/serial/texture.dds", generated_dx10_source_path().string())
                    .has_value());
