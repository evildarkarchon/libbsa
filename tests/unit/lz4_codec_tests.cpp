...
TEST_CASE("lz4_codec frame exact-to-sink writes bounded chunks", "[unit][compression][lz4][sink]")
{
  ...
  auto decoded = libbsa::detail::decompress_lz4_frame_exact_to_sink(input, 1U, frame.value().size(),
                                                                    original.size(), sink, chunk_size,
          ...
          "[unit][malformed][compression][lz4][sink]")
{
  ...
  SECTION("truncated frame")
  {
    ...
    auto decoded = libbsa::detail::decompress_lz4_frame_exact_to_sink(input, 0U, frame.size(), original.size(), sink,
                                                                      13U, "truncated LZ4 frame payload");
  ...
  SECTION("decoded output exceeds metadata")
  {
    ...
    auto decoded = libbsa::detail::decompress_lz4_frame_exact_to_sink(input, 0U, frame.size(), declared_size, sink,
                                                                      13U, "overproducing LZ4 frame payload");
  ...
  SECTION("raw LZ4 block is not a frame")
  {
    ...
    auto decoded = libbsa::detail::decompress_lz4_frame_exact_to_sink(input, 0U, raw_block.size(), original.size(), sink,
                                                                      13U, "non-frame LZ4 payload");
