...
TEST_CASE(
    "tes4 raw table seam reports table sizing and folder file-name "
    "offset handling",
...
TEST_CASE(
    ...
    auto read_v103_payload = [&v103_bytes](
                                 std::uint64_t offset,
                                 std::size_t count) -> libbsa::result<std::vector<std::byte>> {
        const auto start = static_cast<std::size_t>(offset);
        return std::vector<std::byte>{
            v103_bytes.begin() + static_cast<std::ptrdiff_t>(start),
    ...
    auto read_v104_payload = [&v104_bytes](
        ...
        const auto start = static_cast<std::size_t>(offset);
        return std::vector<std::byte>{
            v104_bytes.begin() + static_cast<std::ptrdiff_t>(start),
    ...
    auto read_v105_payload = [&v105_bytes](
        ...
        const auto start = static_cast<std::size_t>(offset);
        return std::vector<std::byte>{
            v105_bytes.begin() + static_cast<std::ptrdiff_t>(start),
