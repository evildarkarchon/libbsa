...
std::filesystem::path seam_output_path(std::string name) { return seam_test_dir() / std::move(name); }
...
libbsa::formats::ba2::ba2_dx10_writer_entry snapshot_entry(std::string archive_path,
  ...
  for (std::uint32_t array_index = 0; array_index < layout.array_size; ++array_index) {
    for (std::uint32_t face_index = 0; face_index < face_count; ++face_index) {
      for (std::uint32_t mip = 0; mip < layout.mip_count; ++mip) {
        ...
        const auto snapshot = seam_output_path(snapshot_prefix + "-" + std::to_string(array_index) + "-" +
...
std::vector<std::byte> expected_chunk_bytes(const libbsa::texture::dds_texture_layout& layout,
  ...
  for (std::uint32_t mip = planned.start_mip; mip <= planned.end_mip; ++mip) {
    ...
    REQUIRE(mip_size.has_value());
    auto bytes = repeated_bytes(static_cast<std::size_t>(mip_size.value()),
                                static_cast<std::uint8_t>(planned.array_index * 31U + planned.face_index * 7U + mip + 1U));
...
void require_decoded_chunk_matches(const libbsa::formats::ba2::ba2_dx10_prepared_chunk& chunk,
                                   const libbsa::texture::dds_texture_layout& layout,
                                   const libbsa::texture::planned_texture_chunk& planned) {
  auto decoded = libbsa::detail::decompress_payload_exact(chunk.compression,
                                                          chunk.stored_payload,
                                                          static_cast<std::size_t>(planned.raw_size));
...
TEST_CASE("BA2 DX10 snapshot builder preserves add-time snapshot immutability and target validation",
  ...
  auto built = libbsa::formats::ba2::ba2_dx10_build_writer_entry_snapshot(
      source_case.at("archive_path").get<std::string>(), scratch_path.string(), libbsa::ba2_dx10_target::fallout4,
      snapshot_dir, 7U);
  ...
  for (std::size_t index = 0; index < built.value().subresources.size(); ++index) {
    ...
    CHECK(read_binary_file(snapshot.snapshot_path) == original_source.value().subresources[index].bytes);
  }
  ...
  auto unsupported = libbsa::formats::ba2::ba2_dx10_build_writer_entry_snapshot(
      ...
      (generated_source_dir() / starfield_only.at("file").get<std::string>()).string(), libbsa::ba2_dx10_target::fallout4,
      snapshot_dir, 8U);
  REQUIRE_FALSE(unsupported.has_value());
  CHECK(unsupported.error().code == libbsa::error_code::format_error);
}

TEST_CASE("BA2 DX10 chunk assembler streams snapshot-backed chunk assembly in texture order",
          "[unit][writer-stage][ba2_dx10_writer][parser_preparer_seam]") {
  SECTION("multi-mip ordering") {
    ...
    auto chunk = libbsa::formats::ba2::ba2_dx10_assemble_chunk(libbsa::ba2_dx10_target::fallout4,
...
TEST_CASE("BA2 DX10 chunk assembler fails on missing or truncated snapshot bytes before publish-visible output changes",
...
TEST_CASE("BA2 DX10 chunk assembler preserves Fallout 4 and Starfield v3 compression routing",
          "[unit][writer-stage][ba2_dx10_writer][parser_preparer_seam]") {
  ...
  starfield_deflate_options.starfield_compression_method = libbsa::formats::ba2::ba2_starfield_compression_deflate;
  auto starfield_deflate = libbsa::formats::ba2::ba2_dx10_assemble_chunk(libbsa::ba2_dx10_target::starfield_v3,
                                                                         starfield_deflate_options,
                                                                         source,
                                                                         planned.value()[0]);
  ...
  starfield_lz4_options.starfield_compression_method = libbsa::formats::ba2::ba2_starfield_compression_lz4_block;
  auto starfield_lz4 = libbsa::formats::ba2::ba2_dx10_assemble_chunk(libbsa::ba2_dx10_target::starfield_v3,
                                                                     starfield_lz4_options,
                                                                     source,
                                                                     planned.value()[0]);
