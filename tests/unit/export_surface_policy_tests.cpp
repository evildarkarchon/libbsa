...
TEST_CASE("export_surface public export header documents macro modes",
...
TEST_CASE("export_surface public emitted APIs are explicitly annotated",
          "[unit][public-api][export_surface][export_surface_policy]") {
    ...
    REQUIRE(validation_header.find("LIBBSA_API bool is_valid() const noexcept") !=
            std::string::npos);
    REQUIRE(validation_header.find("LIBBSA_API result<validation_report> validate_archive") !=
            std::string::npos);
}
