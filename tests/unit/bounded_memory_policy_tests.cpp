...
TEST_CASE(
    "bounded_memory_policy static boundary keeps BSA writer "
    "implementations independent of TES5Edit",
...
TEST_CASE(
    "bounded_memory_policy static boundary keeps BA2 writer implementations "
    "independent of TES5Edit",
...
TEST_CASE(
    "bounded_memory_policy public API boundary hides private BSA writer "
    "helpers",
    "[unit][bounded_memory_policy][static_boundary][public-api]") {
    constexpr auto forbidden_tokens = std::array<std::string_view, 6U>{
        "tes3_writer_entry",      "tes4_writer_entry",      "prepared_entry",
        "write_tes3_bsa_archive", "write_tes4_bsa_archive", "formats::bsa",
    };
    ...
    for (const auto& entry : std::filesystem::directory_iterator{include_dir}) {
        ...
        for (const auto token : forbidden_tokens) {
            INFO("public BSA writer helper boundary token: " << token << " in "
                                                             << entry.path().string());
...
TEST_CASE(
    "bounded_memory_policy public API boundary hides private BA2 writer "
    "helpers",
    "[unit][bounded_memory_policy][static_boundary][public-api][ba2_"
    "writer_execution]") {
    ...
    for (const auto& entry : std::filesystem::directory_iterator{include_dir}) {
        ...
        const auto text = read_text_file(entry.path());
        for (const auto token : forbidden_tokens) {
            INFO("public BA2 writer helper boundary token: " << token << " in "
                                                             << entry.path().string());
            REQUIRE(text.find(token) == std::string::npos);
        }
    }
...
TEST_CASE(
    "bounded_memory_policy public API boundary keeps writer.hpp free of "
    "private codec and reference dependencies",
    "[unit][bounded_memory_policy][static_boundary][public-api][ba2_"
    "writer_execution]") {
    constexpr auto forbidden_tokens = std::array<std::string_view, 5U>{
        "DirectXTex", "DXGI", "libdeflate", "lz4", "TES5Edit",
    };
...
TEST_CASE(
    "bounded_memory_policy targeted writer stages avoid byte-at-a-time "
    "disk source accumulation",
...
TEST_CASE(
    "bounded_memory_policy BA2 DX10 snapshot temp directories use "
    "hardened reservation",
    "[unit][bounded_memory_policy][ba2_dx10_writer][security]") {
    const auto text =
        read_text_file(source_root() / "src" / "formats" / "ba2" / "ba2_dx10_prepare.cpp") +
