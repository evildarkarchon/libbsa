...
std::filesystem::path output_path(std::string_view name) {
    ...
    auto directory = bulk_extraction_test_dir() /
...
libbsa::archive_reader create_bulk_test_reader() {
    ...
    REQUIRE(
        writer.add_bytes("Textures/Beta.DDS", bytes_from_text("beta texture bytes")).has_value());
    REQUIRE(
        writer.add_bytes("Docs/Gamma.txt", bytes_from_text("gamma document bytes")).has_value());
...
class recording_sink_factory final : public libbsa::bulk_extract_sink_factory {
   public:
    ...
    explicit recording_sink_factory(std::string failing_path)
        : failing_path_(std::move(failing_path)) {}
...
void require_matching_entry_metadata(const libbsa::entry_metadata& expected,
                                     const libbsa::entry_metadata& actual) {
...
std::vector<std::byte> build_raw_ba2_dx10_fixture(std::string_view archive_path,
    ...
    const auto directory =
        slash == std::string_view::npos ? std::string_view{} : archive_path.substr(0U, slash);
...
static_assert(
    std::is_same_v<decltype(std::declval<libbsa::bulk_extract_sink_factory&>().create(
...
static_assert(std::is_same_v<decltype(libbsa::bulk_extract_entry_result{}.failure),
                             std::optional<libbsa::error>>);
...
TEST_CASE("bulk_extraction rejects zero workers as an outer argument error",
...
TEST_CASE(
    "bulk_extraction rejects unsupported large worker counts as an outer "
    "argument error",
    "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const auto requests = standard_requests();
...
TEST_CASE(
    "bulk_extraction preserves request order and bytes in serial and "
    "parallel mode",
    "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const auto requests = standard_requests();
    ...
    CHECK(serial.captures.sink_bytes_by_path.at("Docs/Gamma.txt") ==
          single_sink_bytes("gamma document bytes"));
    CHECK(serial.captures.sink_bytes_by_path.at("Meshes/Alpha.NIF") ==
          single_sink_bytes("alpha mesh bytes"));
    CHECK(serial.captures.sink_bytes_by_path.at("Textures/Beta.DDS") ==
          single_sink_bytes("beta texture bytes"));
...
TEST_CASE("bulk_extraction coalesces duplicate successes in serial mode",
          "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    ...
    CHECK(report.sink_bytes_by_path.at("Meshes/Alpha.NIF") ==
          single_sink_bytes("alpha mesh bytes"));
    CHECK(report.sink_bytes_by_path.at("Textures/Beta.DDS") ==
          single_sink_bytes("beta texture bytes"));
}

TEST_CASE("bulk_extraction coalesces duplicate successes in parallel mode",
          "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    ...
    auto extracted = reader.extract_entries(requests, sink_factory,
                                            libbsa::bulk_extract_options{.worker_count = 4U});
    ...
    CHECK(create_count_for(report, "Meshes/Alpha.NIF") == 1U);
    CHECK(create_count_for(report, "Textures/Beta.DDS") == 1U);
    CHECK(report.sink_bytes_by_path.at("Docs/Gamma.txt") ==
          single_sink_bytes("gamma document bytes"));
    CHECK(report.sink_bytes_by_path.at("Meshes/Alpha.NIF") ==
          single_sink_bytes("alpha mesh bytes"));
    CHECK(report.sink_bytes_by_path.at("Textures/Beta.DDS") ==
...
TEST_CASE(
    "bulk_extraction mirrors duplicate missing path failures without "
    "aborting siblings",
    "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
...
TEST_CASE(
    "bulk_extraction mirrors duplicate sink factory failures without "
    "aborting siblings",
    "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const std::vector requests{libbsa::bulk_extract_request{.path = "Meshes/Alpha.NIF"},
...
TEST_CASE("bulk_extraction does not pre-merge distinct request strings",
          "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const std::vector requests{libbsa::bulk_extract_request{.path = "Meshes/Alpha.NIF"},
                               libbsa::bulk_extract_request{.path = "meshes\\alpha.nif"}};
...
TEST_CASE("bulk_extraction records per-entry failures without aborting siblings",
          "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const std::vector requests{libbsa::bulk_extract_request{.path = "Meshes/Alpha.NIF"},
                               libbsa::bulk_extract_request{.path = "Missing/Entry.bin"},
...
TEST_CASE("bulk_extraction records partial sink writes as per-entry I/O errors",
          "[unit][bulk_extraction]") {
    const auto reader = create_bulk_test_reader();
    const std::vector requests{libbsa::bulk_extract_request{.path = "Meshes/Alpha.NIF"}};
...
TEST_CASE("bulk_extraction streams large raw archive families through bounded chunks",
    ...
    require_bulk_extracts_in_bounded_chunks(create_tes3_raw_reader(large_payload),
                                            "Meshes/LargeRaw.NIF", large_payload.size());
