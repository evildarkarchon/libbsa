...
constexpr std::wstring_view non_ascii_host_path_token =
    L"libbsa-\u00C5ngstr\u00F6m-\u65E5\u672C\u8A9E";
...
std::filesystem::path unique_non_ascii_host_path() {
    ...
    return std::filesystem::temp_directory_path() /
           std::filesystem::path{std::wstring{non_ascii_host_path_token} + L"-" +
...
TEST_CASE("host_file_path decodes UTF-8 public host text into the native path",
          "[unit][host_file_path]") {
...
TEST_CASE("host_file_path rejects embedded NUL bytes before filesystem I/O",
          "[unit][host_file_path]") {
