...
std::vector<std::byte> read_binary_file_span(const std::filesystem::path& path,
...
std::uint32_t read_u32_le(const std::vector<std::byte>& bytes, std::size_t offset) {
    return static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes.at(offset))) |
           (static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes.at(offset + 1U)))
            << 8U) |
           (static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes.at(offset + 2U)))
            << 16U) |
           (static_cast<std::uint32_t>(std::to_integer<unsigned char>(bytes.at(offset + 3U)))
            << 24U);
}
...
synthetic_tes3_archive build_synthetic_tes3_archive(std::vector<synthetic_tes3_entry> entries,
                                                    bool sort_by_hash = true) {
    ...
    for (const auto& entry : entries) {
        name_table_size +=
            checked_test_u32(entry.path.size() + 1U, "TES3 synthetic name table size");
    }
    ...
    const auto records_size =
        checked_test_u32(entries.size() * 8U, "TES3 synthetic file records size");
    const auto name_offsets_size =
        checked_test_u32(entries.size() * 4U, "TES3 synthetic name offsets size");
    const auto hash_records_size =
        checked_test_u32(entries.size() * 8U, "TES3 synthetic hash records size");
    const auto hash_table_start =
        checked_test_u32(fixed_header_size + records_size + name_offsets_size + name_table_size,
                         "TES3 synthetic hash table start");
    const auto data_section_start =
        checked_test_u32(hash_table_start + hash_records_size, "TES3 synthetic data section start");

    std::size_t payload_bytes_size = 0U;
    for (const auto& entry : entries) {
        payload_bytes_size = std::max(
            payload_bytes_size, static_cast<std::size_t>(entry.raw_offset) + entry.payload.size());
    }
    std::vector<std::byte> payload_bytes(payload_bytes_size, std::byte{0});
    for (const auto& entry : entries) {
        std::copy(entry.payload.begin(), entry.payload.end(),
                  payload_bytes.begin() + static_cast<std::ptrdiff_t>(entry.raw_offset));
    }
...
TEST_CASE("tes3_bsa_metadata opens generated Morrowind archives",
          "[unit][fixture][tes3_bsa_metadata]") {
    const auto manifest = read_json_file(generated_archive_path("tes3_success_manifest.json"));
...
TEST_CASE("tes3_bsa_entries exposes manifest-backed metadata and offsets",
          "[unit][fixture][tes3_bsa_entries]") {
    const auto manifest = read_json_file(generated_archive_path("tes3_success_manifest.json"));
    auto opened = libbsa::archive_reader::open(generated_archive_path("tes3_success.bsa").string());
    REQUIRE(opened.has_value());
    ...
    for (std::size_t index = 0; index < entries.value().size(); ++index) {
        ...
        REQUIRE(actual.payload_offset ==
                manifest.at("data_section_start").get<std::uint64_t>() +
...
TEST_CASE("tes3_bsa_lookup normalizes variants and stable missing-path behavior",
...
TEST_CASE(
    ...
    for (const auto& expected : manifest.at("entries")) {
        ...
        const auto archive_bytes =
            read_binary_file_span(generated_archive_path("tes3_success.bsa"),
...
TEST_CASE("tes3_bsa_extract helper is raw-only and enforces sink writes",
          "[unit][tes3_bsa_extract]") {
    const std::array payload{std::byte{'r'}, std::byte{'a'}, std::byte{'w'}};
    const libbsa::entry_metadata raw_entry{"raw/path.txt",
                                           "Raw/Path.txt",
                                           3U,
                                           3U,
                                           0U,
    ...
    const auto host_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_raw_payload_source.bsa";
    ...
    auto extracted =
        libbsa::formats::bsa::extract_tes3_bsa_payload(resolved_host_path.value(), raw_entry, sink);

    REQUIRE(extracted.has_value());
    REQUIRE(sink.bytes() == std::vector<std::byte>{payload.begin(), payload.end()});

    partial_sink partial;
    auto partial_result = libbsa::formats::bsa::extract_tes3_bsa_payload(resolved_host_path.value(),
                                                                         raw_entry, partial);
    ...
    const auto large_host_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_large_payload_source.bsa";
    write_binary_file(large_host_path, large_payload);
    const libbsa::entry_metadata large_entry{"large/path.bin",
                                             "Large/Path.bin",
                                             large_payload.size(),
                                             large_payload.size(),
                                             0U,
    ...
    auto resolved_large_host_path =
        libbsa::detail::resolve_host_file_path(large_host_path.string());
    REQUIRE(resolved_large_host_path.has_value());

    auto large_result = libbsa::formats::bsa::extract_tes3_bsa_payload(
        resolved_large_host_path.value(), large_entry, recording);

    REQUIRE(large_result.has_value());
    REQUIRE(recording.write_sizes.size() > 1U);
    REQUIRE(std::all_of(recording.write_sizes.begin(), recording.write_sizes.end(),
                        [](std::size_t size) { return size <= expected_chunk_ceiling; }));

    const libbsa::entry_metadata compressed_entry{"raw/path.txt",
                                                  "Raw/Path.txt",
                                                  3U,
                                                  3U,
                                                  0U,
    ...
    auto compressed_result = libbsa::formats::bsa::extract_tes3_bsa_payload(
        resolved_host_path.value(), compressed_entry, compressed_sink);
...
TEST_CASE(
    "tes3_bsa_malformed rejects generated malformed TES3 cases with "
    "stable error codes",
    ...
    const std::vector<std::string> required_case_ids{
        "tes3_truncated_header",         "tes3_truncated_records",
        "tes3_invalid_name_span",        "tes3_invalid_payload_span",
        "tes3_duplicate_canonical_path", "tes3_inconsistent_counts_offsets",
        "tes3_stored_hash_mismatch",     "tes3_hash_collision",
        "tes3_unsorted_hash_records",    "tes3_raw_offset_absolute_regression",
    };

    for (const auto& required_id : required_case_ids) {
        REQUIRE(std::any_of(manifest.at("cases").begin(), manifest.at("cases").end(),
...
TEST_CASE("tes3_bsa_malformed rejects overlapping non-empty payload spans",
    ...
    const auto archive_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_overlapping_payload_spans.bsa";
...
TEST_CASE("tes3_bsa_entries accepts adjacent spans and zero-byte boundary entries",
    ...
    const auto archive_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_adjacent_and_empty_spans.bsa";
    write_binary_file(archive_path, archive.bytes);
...
TEST_CASE("tes3_bsa_malformed reports unsorted hashes before payload overlap",
    ...
    const auto archive_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_hash_precedes_overlap.bsa";
    write_binary_file(archive_path, archive.bytes);
...
TEST_CASE(
    "tes3_bsa_entries materializes large non-overlapping spans outside "
    "payload order",
    "[unit][tes3_bsa_metadata]") {
    ...
    REQUIRE_FALSE(std::is_sorted(
        entries.begin(), entries.end(),
        [](const auto& lhs, const auto& rhs) { return lhs.raw_offset < rhs.raw_offset; }));

    auto archive = build_synthetic_tes3_archive(std::move(entries), false);
    const auto archive_path =
        std::filesystem::temp_directory_path() / "libbsa_tes3_large_reverse_payload_order.bsa";
    write_binary_file(archive_path, archive.bytes);
...
TEST_CASE(
    "tes3_bsa_malformed returns format_error for oversized declared "
    "metadata without exceptions",
    ...
    const auto mutated =
        std::filesystem::temp_directory_path() / "libbsa_tes3_oversized_metadata.bsa";
...
TEST_CASE("tes3_bsa_malformed rejects declared file counts above the metadata limit",
          "[unit][fixture][malformed][tes3_bsa_malformed]") {
    auto bytes = read_binary_file(generated_archive_path("tes3_success.bsa"));
    overwrite_u32_le(bytes, 8U,
                     static_cast<std::uint32_t>(libbsa::detail::metadata_entry_count_limit + 1U));

    const auto mutated =
        std::filesystem::temp_directory_path() / "libbsa_tes3_excessive_file_count.bsa";
    write_binary_file(mutated, bytes);
...
TEST_CASE("tes3_bsa_malformed maps invalid archive names to format_error",
          "[unit][fixture][malformed][tes3_bsa_malformed]") {
    auto bytes = read_binary_file(generated_archive_path("tes3_success.bsa"));
    ...
    std::string original;
    for (auto offset = first_name_start; offset < bytes.size() && bytes.at(offset) != std::byte{0};
         ++offset) {
    ...
    std::transform(mutated_name.begin(), mutated_name.end(),
                   bytes.begin() + static_cast<std::ptrdiff_t>(first_name_start), [](char value) {
