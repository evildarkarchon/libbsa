          ...
          "[unit][fixture][ba2_dx10_metadata]")
{
  ...
  STATIC_REQUIRE(std::is_same_v<decltype(texture.chunks), std::vector<libbsa::texture_chunk_metadata>>);
...
TEST_CASE("ba2_dx10_metadata keeps non-texture entries opt-in", "[unit][fixture][ba2_dx10_metadata]")
{
  ...
  STATIC_REQUIRE(std::is_same_v<decltype(entry.texture), std::optional<libbsa::texture_metadata>>);
