...
TEST_CASE("binary_io reader reports malformed truncation without overread",
          "[unit][malformed][binary-io]") {
...
TEST_CASE("binary_io reader read_bytes advances only on success", "[unit][malformed][binary-io]") {
    const auto bytes =
        std::to_array<std::byte>({std::byte{0x01}, std::byte{0x02}, std::byte{0x03}});
    binary_reader reader{bytes};
