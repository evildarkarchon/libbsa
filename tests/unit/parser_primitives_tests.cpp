...
TEST_CASE("parser_primitives validate checked arithmetic", "[unit][parser_primitives]") {
    ...
    // BA2 host-file parsing uses this contract for FileTableOffset plus parsed
    // filename-table bytes; a full fixture cannot usually seek to offsets high
    // enough to overflow UInt64 before stream limits reject it.
    ...
    REQUIRE_FALSE(
        libbsa::detail::add_fits_u64(std::numeric_limits<std::uint64_t>::max(), 1U, total64));
}

TEST_CASE("parser_primitives reject spans outside archive bounds",
          "[unit][parser_primitives][malformed]") {
...
TEST_CASE("parser_primitives enforce metadata count limits",
          "[unit][parser_primitives][malformed]") {
    auto accepted = libbsa::detail::validate_metadata_count(
        libbsa::detail::metadata_entry_count_limit, libbsa::detail::metadata_entry_count_limit,
        "test metadata count");
    REQUIRE(accepted.has_value());

    auto rejected = libbsa::detail::validate_metadata_count(
        libbsa::detail::metadata_entry_count_limit + 1U, libbsa::detail::metadata_entry_count_limit,
        "test metadata count");
    REQUIRE_FALSE(rejected.has_value());
    REQUIRE(rejected.error().code == libbsa::error_code::format_error);
}

TEST_CASE("parser_primitives translate typed metadata reserve failures",
          "[unit][parser_primitives][allocation]") {
    std::vector<std::uint64_t> values;
    auto vector_reserved =
        libbsa::detail::reserve_metadata_vector(values, 4U, "test metadata vector");
    REQUIRE(vector_reserved.has_value());
    REQUIRE(values.capacity() >= 4U);

    auto impossible_vector = libbsa::detail::reserve_metadata_vector(
        values, impossible_vector_capacity<std::uint64_t>(), "test metadata vector");
    ...
    auto impossible_set = libbsa::detail::reserve_metadata_set(
        set, impossible_set_capacity<std::uint64_t>(), "test metadata set");
...
TEST_CASE("parser_primitives read exact bounded file spans", "[unit][parser_primitives]") {
    const auto temp_path =
        std::filesystem::temp_directory_path() / "libbsa_parser_primitives_exact_read.bin";
...
TEST_CASE("parser_primitives reject stream limits before seeking or allocating",
          "[unit][parser_primitives][malformed]") {
    std::ifstream input;

    auto bad_offset = libbsa::detail::read_file_bytes_at(
        input, static_cast<std::uint64_t>(std::numeric_limits<std::streamoff>::max()) + 1U, 0U,
        "test parser span");
    REQUIRE_FALSE(bad_offset);
    REQUIRE(bad_offset.error().code == libbsa::error_code::format_error);

    auto bad_size = libbsa::detail::read_file_bytes_at(
        input, 0U, static_cast<std::size_t>(std::numeric_limits<std::streamsize>::max()) + 1U,
        "test parser span");
...
TEST_CASE("parser_primitives materialize archive strings through result errors",
          "[unit][parser_primitives][allocation]") {
    const std::vector<std::byte> bytes{std::byte{'M'}, std::byte{'e'}, std::byte{'s'},
                                       std::byte{'h'}};
    ...
    auto impossible = libbsa::detail::archive_string_from_bytes(
        std::span<const std::byte>{&source, impossible_string_size()}, "test archive string");
