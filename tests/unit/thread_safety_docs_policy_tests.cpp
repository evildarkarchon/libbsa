...
TEST_CASE("thread_safety_policy public types have canonical documentation sections",
    ...
    constexpr std::array writer_types{
        "tes3_bsa_writer", "tes4_bsa_writer",         "ba2_gnrl_writer",
        "ba2_dx10_writer", "write_execution_options",
    };
...
TEST_CASE(
    "thread_safety_policy documents callback sink writer validation and "
    "benchmark rules",
    "[unit][thread_safety_policy][doc_structure]") {
    const auto root = source_root();
    const auto docs = read_text_file(root / "docs" / "thread-safety.md");
    ...
    require_all_tokens(docs, {"distinct sink",
                              ...
                              "libbsa does not call sink factory or sink methods while holding "
                              "internal locks",
                              "add_file",
                              "add_bytes",
    ...
    require_all_tokens(compatibility_evidence,
                       {"Generated legal fixtures", "writer-output archives", "mandatory evidence",
                        "public compatibility warnings"});
    require_all_tokens(fixture_policy,
                       {"Compatibility evidence", "generated fixtures", "writer-output archives",
                        "Benchmark and generated data policy", "generated legal synthetic data"});
}
...
TEST_CASE("thread_safety_policy public headers point to canonical guidance",
    ...
    require_all_tokens(archive_header, {"Thread-safety", "docs/thread-safety.md",
                                        "caller-owned synchronization", "distinct sinks",
                                        "Duplicate exact request paths", "unique-path extraction"});
    require_all_tokens(writer_header,
                       {"Thread-safety", "docs/thread-safety.md", "write-call execution controls",
                        "mutation is not concurrent"});
    require_all_tokens(validation_header,
                       {"Thread-safety", "docs/thread-safety.md", "no global mutable state"});
}
