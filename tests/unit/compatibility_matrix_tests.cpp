...
void require_required_matrix_keys(const nlohmann::json& row) {
    for (const auto key :
         {"id", "family", "category", "evidence_type", "phase", "expected_error"}) {
...
TEST_CASE("compatibility_matrix schema spans malformed families and risk categories",
    ...
    const std::set<std::string> required_categories{
        "truncated_structure", "duplicate_canonical_path", "invalid_payload_span",
        "unsupported_route",   "decompression_failure",    "oversized_arithmetic",
        "dds_chunk_layout"};
...
TEST_CASE(
    "compatibility_matrix evidence references resolve to manifests or "
    "test tokens",
    "[unit][fixture][malformed][compatibility_matrix]") {
    REQUIRE(std::filesystem::is_regular_file(compatibility_matrix_path()));
    const auto matrix = read_json_file(compatibility_matrix_path());
    ...
    for (const auto& row : matrix.at("rows")) {
        ...
        if (evidence_type == "manifest") {
            ...
            const auto manifest_path =
                generated_archive_dir() / row.at("manifest").get<std::string>();
