...
void require_same_reopened_payloads(
    const std::filesystem::path& serial_output, const std::filesystem::path& parallel_output,
...
void add_tes4_sources(
    libbsa::tes4_bsa_writer& writer, const std::filesystem::path& root,
    std::span<const std::pair<std::string_view, std::vector<std::byte>>> entries) {
...
TEST_CASE(
    "bsa_writer_execution TES3 worker_count streams large disk-backed source "
    "and reopens output",
...
TEST_CASE(
    "bsa_writer_execution TES4 worker_count preserves deflate and LZ4-frame "
    "output compatibility",
    "[unit][bsa_writer_execution][bounded_memory_policy][tes4_bsa_writer]") {
    constexpr std::array targets{libbsa::tes4_bsa_target::fallout3,
    ...
    const std::array entries{
        ...
        std::pair<std::string_view, std::vector<std::byte>>{
            "Scripts/Small/Quest.pex", bytes_from_text("small script payload")},
    };
...
TEST_CASE(
    "bsa_writer_execution missing disk source with worker_count "
...
TEST_CASE(
    "bsa_writer_execution overwrite failure with worker_count preserves "
    "readable previous archive",
    "[unit][bsa_writer_execution][bounded_memory_policy][tes4_bsa_writer]"
    "[publish]") {
    ...
    REQUIRE(
        original_writer.add_bytes("Meshes/Original/Readable.nif", original_payload).has_value());
    ...
    libbsa::tes4_bsa_writer replacement_writer{libbsa::tes4_bsa_target::fallout3,
                                               replacement_options};
    REQUIRE(replacement_writer.add_file("Meshes/Missing/Replacement.nif", missing_source.string())
                .has_value());
