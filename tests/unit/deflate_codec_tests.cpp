...
TEST_CASE("deflate_codec round trips exact-size payloads", "[unit][compression][deflate]") {
    ...
    REQUIRE(compressed);
    auto decompressed =
        libbsa::detail::decompress_deflate_exact(compressed.value(), original.size());

    REQUIRE(decompressed);
    REQUIRE(decompressed.value() == original);
}

TEST_CASE("deflate_codec rejects truncated and exact-size mismatches",
          "[unit][malformed][compression][deflate]") {
    const auto original = deflate_vector();
    auto compressed = libbsa::detail::compress_deflate(original);
    REQUIRE(compressed);
    ...
    auto too_large =
        libbsa::detail::decompress_deflate_exact(compressed.value(), original.size() + 1);
...
TEST_CASE("deflate_codec translates impossible output allocations",
          "[unit][malformed][compression][deflate][allocation]") {
    auto allocated =
        libbsa::detail::make_byte_vector(impossible_byte_vector_size(), "test deflate allocation");
