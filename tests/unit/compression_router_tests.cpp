...
namespace
{
  ...
  {
  public:
    /// Records each accepted write so router tests can verify exact bytes and chunk bounds.
...
TEST_CASE("compression_router sink route streams LZ4 frame payloads", "[unit][compression][compression-router][sink]")
{
  constexpr std::size_t chunk_size = 19U;
  const auto original = router_vector();
  auto compressed = libbsa::detail::compress_payload(libbsa::detail::compression_method::lz4_frame, original);
  REQUIRE(compressed);
  ...
  auto decoded = libbsa::detail::decompress_payload_exact_to_sink(
      libbsa::detail::compression_method::lz4_frame, input, 0U, compressed.value().size(), original.size(), sink,
      chunk_size, "router LZ4 frame payload");
          ...
          "[unit][compression][compression-router][sink]")
{
                            ...
                            libbsa::detail::compression_method::lz4_block})
  {
    ...
    auto decoded = libbsa::detail::decompress_payload_exact_to_sink(method, input, 0U, compressed.value().size(),
                                                                    original.size(), sink, 23U,
    ...
    auto mismatch = libbsa::detail::decompress_payload_exact_to_sink(method, mismatch_input, 0U,
                                                                     compressed.value().size(), original.size() - 1U,
                                                                     mismatch_sink, 23U,
                                                                     "router fallback payload mismatch");
...
TEST_CASE("compression_router rejects unsupported explicit enum values", "[unit][malformed][compression][compression-router]")
{
  ...
  REQUIRE(input.good());
  recording_sink sink;
  auto sink_decoded = libbsa::detail::decompress_payload_exact_to_sink(unsupported, input, 0U, original.size(),
                                                                       original.size(), sink, 8U,
