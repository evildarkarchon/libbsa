...
namespace
{
  ...
  std::filesystem::path unique_non_ascii_root()
  {
    ...
    return std::filesystem::temp_directory_path() / std::filesystem::path{std::wstring{non_ascii_path_token_wide} + L"-host-path-" + std::to_wstring(counter.fetch_add(1, std::memory_order_relaxed))};
  }
  ...
  std::filesystem::path copied_archive_path(const representative_archive_case &archive_case, const std::filesystem::path &root)
  {
    ...
    const auto renamed = std::filesystem::path{source.stem().wstring() + L"-" + std::wstring{non_ascii_path_token_wide} + source.extension().wstring()};
    return root / renamed;
  }
          ...
          "[unit][host_path_correctness_boundary][host_path_correctness_boundary_smoke]")
{
  ...
  const auto open_call = std::string{"archive_reader::"
                                     "open(host_path)"};
  const auto validate_call = std::string{"validate_"
                                         "archive(host_path, options)"};
  const auto extract_bytes_call = std::string{"extract_"
                                              "bytes(canonical_path)"};
  const auto writer_publish_call = std::string{"write_"
                                               "to("};
  const auto writer_add_file_call = std::string{"add_"
                                                "file("};
