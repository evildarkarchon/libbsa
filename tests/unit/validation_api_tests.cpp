...
class temp_file_cleanup final {
   public:
    ...
    /// Best-effort cleanup keeps sparse validation fixtures from surviving failed
    /// tests.
    ~temp_file_cleanup() {
...
void require_metadata_matches_case(const libbsa::validation_report& report,
    ...
    if (test_case.expected_type == libbsa::archive_type::ba2) {
        ...
        if (test_case.expected_ba2_compression_method.has_value()) {
            ...
            CHECK(report.metadata->ba2->compression_method.value() ==
...
bool report_has_error_code(const libbsa::validation_report& report,
                           libbsa::error_code expected_code) {
    return std::any_of(report.errors.begin(), report.errors.end(),
                       [expected_code](const libbsa::validation_diagnostic& error) {
...
std::filesystem::path write_tes3_archive() {
    ...
    REQUIRE(
        writer.add_bytes("Meshes/Validation/TES3.NIF", bytes_from_text("tes3 validation payload"))
            .has_value());
...
std::filesystem::path write_tes4_archive() {
    ...
    REQUIRE(
        writer.add_bytes("Meshes/Validation/TES4.NIF", bytes_from_text("tes4 validation payload"))
            .has_value());
    REQUIRE(writer.write_to(output.string()).has_value());
    return output;
}
...
std::filesystem::path write_ba2_gnrl_archive() {
    ...
    REQUIRE(
        writer
            .add_bytes("Meshes/Validation/Gnrl.nif", bytes_from_text("ba2 gnrl validation payload"))
            .has_value());
    REQUIRE(writer.write_to(output.string()).has_value());
    return output;
}
...
std::filesystem::path write_ba2_gnrl_starfield_v3_archive(std::uint32_t compression_method) {
    ...
    REQUIRE(writer
                .add_bytes("Data/Validation/StarfieldRoute.bin",
...
std::filesystem::path write_ba2_dx10_starfield_v3_archive(std::uint32_t compression_method) {
    const auto stem =
        std::string{"validation-api-ba2-dx10-sfv3-method"} + std::to_string(compression_method);
    const auto output = unique_output_path(stem, ".ba2");
    libbsa::ba2_dx10_writer_options options;
    options.overwrite_existing = true;
    options.starfield_compression_method = compression_method;
    ...
    const auto dds_source = generated_source_dir() / "ba2_dx10_bc1_unorm.dds";
    REQUIRE(writer.add_file("textures/validation/starfield_route.dds", dds_source.string())
                .has_value());
    REQUIRE(writer.write_to(output.string()).has_value());
    return output;
}
...
void require_starfield_v3_ba2_route(std::string_view route_name,
    ...
    const auto report = require_valid_archive(archive_path.string(), libbsa::archive_type::ba2,
...
TEST_CASE("validation_api accepts generated fixture archives", "[unit][fixture][validation_api]") {
    ...
    constexpr std::array cases{
        validation_archive_case{"tes3_success.bsa", libbsa::archive_type::bsa,
                                libbsa::archive_variant::tes3, 0x00000100U,
                                libbsa::entry_compression::none},
        validation_archive_case{"tes4_v103.bsa", libbsa::archive_type::bsa,
                                libbsa::archive_variant::tes4, 103U,
                                libbsa::entry_compression::deflate},
        validation_archive_case{"tes4_v104.bsa", libbsa::archive_type::bsa,
                                libbsa::archive_variant::tes4, 104U,
                                libbsa::entry_compression::deflate},
        validation_archive_case{"tes4_v105.bsa", libbsa::archive_type::bsa,
                                libbsa::archive_variant::tes4, 105U,
                                libbsa::entry_compression::lz4_frame},
        validation_archive_case{"ba2_gnrl_fo4.ba2", libbsa::archive_type::ba2,
                                libbsa::archive_variant::fallout4, 1U,
                                libbsa::entry_compression::deflate},
        validation_archive_case{"ba2_dx10_fo4.ba2", libbsa::archive_type::ba2,
                                libbsa::archive_variant::fallout4, 1U,
                                libbsa::entry_compression::deflate},
        validation_archive_case{"ba2_gnrl_sfv2.ba2", libbsa::archive_type::ba2,
                                libbsa::archive_variant::starfield, 2U,
                                libbsa::entry_compression::deflate},
        validation_archive_case{
            "ba2_gnrl_sfv3.ba2", libbsa::archive_type::ba2, libbsa::archive_variant::starfield, 3U,
            libbsa::entry_compression::lz4_block, std::optional<std::uint32_t>{3U}},
        validation_archive_case{
            "ba2_dx10_sfv3.ba2", libbsa::archive_type::ba2, libbsa::archive_variant::starfield, 3U,
            libbsa::entry_compression::lz4_block, std::optional<std::uint32_t>{3U}},
    };

    for (const auto& test_case : cases) {
        INFO("generated fixture: " << test_case.file_name);
        const auto report = require_valid_archive(
            (generated_archive_dir() / std::string{test_case.file_name}).string(),
            test_case.expected_type, test_case.expected_variant, options);
...
TEST_CASE("validation_api accepts writer-produced archives", "[unit][roundtrip][validation_api]") {
    ...
    require_valid_archive(write_tes3_archive().string(), libbsa::archive_type::bsa,
                          libbsa::archive_variant::tes3, options);
    require_valid_archive(write_tes4_archive().string(), libbsa::archive_type::bsa,
                          libbsa::archive_variant::tes4, options);
    require_valid_archive(write_ba2_gnrl_archive().string(), libbsa::archive_type::ba2,
...
TEST_CASE(
    "validation_api accepts writer-produced Starfield BA2 v3 compression "
    "routes",
    "[unit][roundtrip][validation_api]") {
    constexpr std::array cases{
        starfield_validation_writer_case{"method 0 deflate", 0U,
                                         libbsa::entry_compression::deflate},
        starfield_validation_writer_case{"method 3 raw LZ4 block", 3U,
                                         libbsa::entry_compression::lz4_block},
    };

    for (const auto& test_case : cases) {
        INFO("BA2 GNRL Starfield v3 " << test_case.route_name);
        require_starfield_v3_ba2_route(
            "GNRL", write_ba2_gnrl_starfield_v3_archive(test_case.compression_method),
            test_case.compression_method, test_case.expected_default_compression);

        INFO("BA2 DX10 Starfield v3 " << test_case.route_name);
        require_starfield_v3_ba2_route(
            "DX10", write_ba2_dx10_starfield_v3_archive(test_case.compression_method),
            test_case.compression_method, test_case.expected_default_compression);
    }
}
...
TEST_CASE("validation_api reports expected variant mismatch as a valid risky warning",
    ...
    const auto report =
        require_valid_archive((generated_archive_dir() / "tes3_success.bsa").string(),
                              libbsa::archive_type::bsa, libbsa::archive_variant::tes3, options);
...
TEST_CASE("validation_api reports malformed archives without lenient readers",
          "[unit][fixture][malformed][validation_api]") {
    require_malformed_open_report("tes3_truncated_header.bsa", libbsa::error_code::format_error);
    require_malformed_open_report("malformed_unsupported_version.bsa",
                                  libbsa::error_code::unsupported);
    require_malformed_open_report("ba2_duplicate_canonical_path.ba2",
                                  libbsa::error_code::format_error);
    require_malformed_open_report("ba2_dx10_truncated_header.ba2",
                                  libbsa::error_code::format_error);
}

TEST_CASE(
    "validation_api reports compatibility_matrix malformed rows as "
    "validation errors",
