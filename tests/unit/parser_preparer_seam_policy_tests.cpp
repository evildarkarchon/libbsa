          ...
          "[unit][parser_preparer_seam_policy]")
{
  ...
  const auto payload_header = read_text_file(root / "src/formats/bsa/tes4_bsa_payload_descriptor.hpp");
  ...
  const auto parse_body = function_body(parser,
                                        "result<tes4_bsa_archive> parse_tes4_bsa_archive_impl(",
                                        "} // namespace");
          ...
          "[unit][parser_preparer_seam_policy]")
{
  ...
  const auto make_entry_body = function_body(prepare,
                                             "result<ba2_dx10_writer_entry> ba2_dx10_make_writer_entry(",
                                             "std::uint32_t ba2_dx10_version_for");
  const auto prepare_chunk_body = function_body(prepare,
                                                "result<ba2_dx10_prepared_chunk> ba2_dx10_prepare_chunk(",
                                                "result<std::vector<ba2_dx10_prepared_entry>> ba2_dx10_prepare_entries(");
  const auto prepare_entries_body = function_body(prepare,
                                                  "result<std::vector<ba2_dx10_prepared_entry>> ba2_dx10_prepare_entries(",
