...
/// scan.
std::set<std::filesystem::path> new_snapshot_directories_since(
...
std::filesystem::path first_snapshot_file(const std::set<std::filesystem::path>& snapshot_dirs) {
    ...
    FAIL(
        "expected a BA2 DX10 snapshot file in the writer-owned snapshot "
        "directory");
}
...
std::array<char, 4> read_ascii4(std::span<const std::byte> bytes, std::size_t offset) {
    REQUIRE(offset + 4U <= bytes.size());
    return {static_cast<char>(bytes[offset]), static_cast<char>(bytes[offset + 1U]),
            static_cast<char>(bytes[offset + 2U]), static_cast<char>(bytes[offset + 3U])};
}
...
ba2_dx10_record_metadata expected_record_metadata_for(std::string_view archive_path) {
    ...
    return {std::string{archive_path}, libbsa::detail::hash_fo4(stem),
            expected_extension_fourcc(extension), libbsa::detail::hash_fo4(directory)};
}

std::vector<ba2_dx10_record_metadata> read_ba2_dx10_record_metadata(
    ...
    for (std::uint32_t index = 0; index < file_count; ++index) {
        ...
        records.push_back({{},
                           read_u32_le(bytes, record_cursor),
                           read_ascii4(bytes, record_cursor + 4U),
    ...
    for (auto& record : records) {
        ...
        record.filename_table_path.assign(reinterpret_cast<const char*>(bytes.data() + name_cursor),
                                          name_size);
...
void require_writer_record_metadata_matches_reference(
    const std::filesystem::path& archive_path,
    ...
    for (const auto archive_path_text : added_archive_paths) {
        ...
        const auto found =
            std::ranges::find_if(records, [&](const ba2_dx10_record_metadata& record) {
...
void require_analyzes_valid_source_case(const nlohmann::json& source_case) {
    ...
    CHECK(analysis.value().metadata.dxgi_format ==
          source_case.at("format_id").get<std::uint32_t>());
    ...
    CHECK(analysis.value().metadata.array_size ==
          source_case.at("array_size").get<std::uint32_t>());
...
const nlohmann::json& invalid_source_case(const nlohmann::json& manifest, std::string_view id) {
    const auto found =
        std::ranges::find_if(manifest.at("invalid_cases"), [id](const nlohmann::json& source_case) {
            return source_case.at("id").get<std::string>() == id;
        });
...
void add_matrix_cases(libbsa::ba2_dx10_writer& writer, const nlohmann::json& manifest,
                      libbsa::ba2_dx10_target target,
    ...
    for (const auto& matrix_case : writer_proof_matrix) {
        ...
        auto added = writer.add_file(
            source_case.at("archive_path").get<std::string>(),
...
void require_metadata_matches_source(const libbsa::texture_metadata& texture,
...
void require_writer_round_trip(libbsa::ba2_dx10_target target,
                               std::uint32_t starfield_compression_method, std::string_view stem,
                               libbsa::entry_compression expected_compression) {
    const auto manifest =
    ...
    for (const auto* source_case : added_cases) {
        added_archive_paths.push_back(
            source_case->at("archive_path").get_ref<const std::string&>());
...
void require_structural_writer_round_trip() {
    ...
    std::vector<const nlohmann::json*> added_cases;
    for (const auto id : {"multi_mip_bc7_unorm", "array_bc5_unorm_2slice",
    ...
    for (const auto* source_case : added_cases) {
        require_reader_backed_entry(opened.value(), *source_case,
    ...
    const auto cubemap_array =
        opened.value().find("textures/structural/cubemap_array_bc1_unorm_12face.dds");
...
TEST_CASE("BA2 DX10 writer DDS source manifest covers locked formats",
          "[unit][fixture][ba2_dx10_writer][dds]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");
    constexpr auto locked_formats = std::to_array<std::uint32_t>(
        {28U, 71U, 72U, 77U, 80U, 83U, 84U, 95U, 96U, 98U, 99U, 29U, 87U, 61U, 31U});
...
TEST_CASE("BA2 DX10 writer DDS source manifest exposes dedicated structural cases",
          "[unit][fixture][ba2_dx10_writer][dds]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");
    ...
    CHECK(multi_mip.at("archive_path").get<std::string>() ==
          "textures/structural/multi_mip_bc7_unorm.dds");
    ...
    CHECK(array.at("archive_path").get<std::string>() ==
          "textures/structural/array_bc5_unorm_2slice.dds");
    ...
    CHECK(cubemap.at("archive_path").get<std::string>() ==
          "textures/structural/cubemap_bc1_unorm_6face.dds");
    ...
    CHECK(cubemap_array.at("file").get<std::string>() ==
          "ba2_dx10_cubemap_array_bc1_unorm_12face.dds");
...
TEST_CASE(
    "BA2 DX10 writer DDS source manifest rejects malformed and "
    "unsupported DDS sources",
    "[unit][fixture][ba2_dx10_writer][dds]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");
...
TEST_CASE("BA2 DX10 writer rejects Starfield-only DDS formats for Fallout 4 archives",
          "[unit][ba2_dx10_writer][add]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");

    for (const auto id : {"bc1_unorm_srgb", "bc5_snorm", "bc6h_uf16", "bc7_unorm_srgb",
        ...
        auto fallout4_added = fallout4_writer.add_file(
            source_case.at("archive_path").get<std::string>(), source_path.string());
        ...
        auto starfield_added = starfield_writer.add_file(
            source_case.at("archive_path").get<std::string>(), source_path.string());
...
TEST_CASE("BA2 DX10 writer records use canonical stem hash and extension metadata",
    ...
    REQUIRE(
        writer
            .add_file(mixed_case_archive_path,
                      (generated_source_dir() / source_case.at("file").get<std::string>()).string())
            .has_value());
...
TEST_CASE("ba2_dx10_writer::add_file rejects an empty DDS host path",
          "[unit][ba2_dx10_writer][add]") {
    libbsa::ba2_dx10_writer writer{libbsa::ba2_dx10_target::fallout4};
...
TEST_CASE(
    "ba2_dx10_writer::add_file rejects a missing DDS host path with "
    "structured error",
    "[unit][ba2_dx10_writer][add]") {
...
TEST_CASE("ba2_dx10_writer::add_file rejects unsupported DDS formats",
          "[unit][ba2_dx10_writer][add]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");
...
TEST_CASE(
    "ba2_dx10_writer::add_file snapshots DDS bytes before later source "
    "file changes",
    ...
    auto added =
        writer.add_file(source_case.at("archive_path").get<std::string>(), scratch_path.string());
    ...
    REQUIRE(opened.has_value());
    auto extracted =
        opened.value().extract_bytes(source_case.at("archive_path").get<std::string>());
    REQUIRE(extracted.has_value());
...
TEST_CASE(
    "BA2 DX10 writer removes snapshot temp directories after successful "
    "write while alive",
    "[unit][ba2_dx10_writer][bounded_memory_policy][cleanup]") {
    const auto before = snapshot_directories();
    const auto manifest =
    ...
    REQUIRE(writer.add_file(source_case.at("archive_path").get<std::string>(), source_path.string())
                .has_value());
...
TEST_CASE(
    ...
    const std::vector<std::byte> sentinel{std::byte{0x53}, std::byte{0x4E}, std::byte{0x41},
                                          std::byte{0x50}};
    write_binary_file(output, sentinel);
    libbsa::ba2_dx10_writer_options options;
...
TEST_CASE(
    "BA2 DX10 writer cleans snapshot directories on output failure and "
    ...
    REQUIRE(
        writer
            .add_file(source_case.at("archive_path").get<std::string>(),
                      (generated_source_dir() / source_case.at("file").get<std::string>()).string())
            .has_value());
    const auto created = new_snapshot_directories_since(before);
...
TEST_CASE(
    "BA2 DX10 writer cleans newly reserved snapshot directory when "
    "add_file fails",
    ...
    auto added = writer.add_file(
        "textures/malformed-cleanup.dds",
        (generated_source_dir() / source_case.at("file").get<std::string>()).string());
...
TEST_CASE(
    "ba2_dx10_writer::add_file accepts duplicate canonical archive paths "
...
TEST_CASE(
    "ba2_dx10_writer reopens fo4 deflate compression archives through "
    "archive_reader",
    "[unit][ba2_dx10_writer][fo4][compression]") {
    // DX10 output is compressed-only by public contract; tests intentionally
...
TEST_CASE(
    "ba2_dx10_writer reopens starfield method 3 raw LZ4 compression "
    "archives through archive_reader",
    "[unit][ba2_dx10_writer][starfield][compression]") {
    libbsa::ba2_dx10_writer_options options;
    options.starfield_compression_method = 3U;
    CHECK(options.starfield_compression_method == 3U);
    require_writer_round_trip(libbsa::ba2_dx10_target::starfield_v3,
                              options.starfield_compression_method, "starfield-v3-dx10-writer",
                              libbsa::entry_compression::lz4_block);
}

TEST_CASE(
    "ba2_dx10_writer reopens starfield method 0 deflate compression "
    "archives through archive_reader",
    "[unit][ba2_dx10_writer][starfield][compression]") {
    libbsa::ba2_dx10_writer_options options;
    options.starfield_compression_method = 0U;
    CHECK(options.starfield_compression_method == 0U);
    require_writer_round_trip(
        libbsa::ba2_dx10_target::starfield_v3, options.starfield_compression_method,
        "starfield-v3-dx10-deflate-writer", libbsa::entry_compression::deflate);
}

TEST_CASE(
    "ba2_dx10_writer preserves multi mip array and cubemap image payload "
    "bytes through extraction",
    "[unit][ba2_dx10_writer][starfield][structural]") {
    require_structural_writer_round_trip();
}

TEST_CASE(
    "BA2 DX10 writer keeps duplicate DDS chunk offsets distinct when "
    "deduplicate_payloads = false",
...
TEST_CASE(
    "BA2 DX10 writer shares duplicate DDS chunk offsets when "
...
TEST_CASE(
    "BA2 DX10 writer refuses to overwrite existing output by default and "
    "preserves bytes",
...
TEST_CASE(
    "BA2 DX10 writer preserves caller-owned temp-name sibling files "
    "during unique temp publish",
...
TEST_CASE(
    "BA2 DX10 writer rejects non-regular overwrite targets without "
    "replacing them",
    "[unit][ba2_dx10_writer][publish][overwrite]") {
    const auto manifest =
        read_json_file(generated_source_dir() / "ba2_dx10_writer_sources_manifest.json");
...
TEST_CASE(
    "BA2 DX10 writer overwrites existing archives when "
