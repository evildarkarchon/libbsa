...
class collecting_sink final : public libbsa::payload_sink {
   public:
    /// Appends every sink write so tests can compare sink extraction against
    /// `extract_bytes`.
...
void require_payload_matches_validated_manifest_order(std::span<const std::byte> extracted,
                                                      const nlohmann::json& expected) {
    REQUIRE(extracted.size() >= dds_dxt10_header_size);
    const auto expected_payload =
        bytes_from_hex(expected.at("expected_dds_payload").at("bytes_hex").get<std::string>());
    REQUIRE(std::vector<std::byte>{
                extracted.begin() + static_cast<std::ptrdiff_t>(dds_dxt10_header_size),
                extracted.end()} == expected_payload);
    ...
    for (const auto& chunk : expected.at("chunks")) {
        ...
        // The manifest records the format-derived validated order: each
        // logical_texture_segment identity maps to the parser's source_chunk_index,
        // proving extraction does not concatenate raw archive bytes.
        INFO("array_index=" << segment.at("array_index").get<std::uint32_t>()
                            << " face_index=" << segment.at("face_index").get<std::uint32_t>()
                            << " source_chunk_index="
                            << segment.at("source_chunk_index").get<std::uint32_t>());
        REQUIRE(segment.at("start_mip").get<std::uint32_t>() ==
                chunk.at("start_mip").get<std::uint32_t>());
        REQUIRE(segment.at("end_mip").get<std::uint32_t>() ==
                chunk.at("end_mip").get<std::uint32_t>());
        REQUIRE(segment.at("source_chunk_index").get<std::uint32_t>() <
                expected.at("chunks").size());
        REQUIRE(extracted.size() >= payload_offset + decoded.size());
        REQUIRE(std::equal(decoded.begin(), decoded.end(),
                           extracted.begin() + static_cast<std::ptrdiff_t>(payload_offset)));
        payload_offset += decoded.size();
    }
    REQUIRE(payload_offset == extracted.size());
}

void require_directxtex_metadata_matches_manifest(std::span<const std::byte> dds_bytes,
                                                  const nlohmann::json& expected) {
...
TEST_CASE("ba2_dx10_extract reconstructs DDS bytes through sink and extract_bytes",
          "[unit][fixture][ba2_dx10_extract]") {
...
TEST_CASE("ba2_dx10_compression routes raw deflate and LZ4 block chunks",
          "[unit][fixture][ba2_dx10_compression]") {
    bool saw_raw = false;
...
TEST_CASE("ba2_dx10_compressed_chunk_fallbacks preserve reconstructed fixture bytes",
    ...
    for (const auto& fixture : success_fixtures()) {
        ...
        for (const auto& expected : manifest.at("entries")) {
            ...
            for (const auto& chunk : expected.at("chunks")) {
                const auto route = chunk.at("compression_route").get<std::string>();
                has_compressed_fallback =
                    has_compressed_fallback || route == "deflate" || route == "lz4_block";
                saw_deflate = saw_deflate || route == "deflate";
                saw_lz4_block = saw_lz4_block || route == "lz4_block";
            }
...
TEST_CASE("ba2_dx10_directxtex validates reconstructed DDS metadata",
          "[unit][fixture][ba2_dx10_directxtex]") {
    for (const auto& fixture : success_fixtures()) {
        const auto manifest = read_json_file(generated_archive_path(fixture.manifest));
        auto opened =
            libbsa::archive_reader::open(generated_archive_path(fixture.archive).string());
...
TEST_CASE("ba2_dx10_extract detects partial_sink writes after header output",
          "[unit][fixture][ba2_dx10_extract]") {
    ...
    auto resolved_host_path =
        libbsa::detail::resolve_host_file_path(generated_archive_path("ba2_dx10_fo4.ba2").string());
    REQUIRE(resolved_host_path.has_value());

    auto result = libbsa::formats::ba2::extract_ba2_dx10_payload(resolved_host_path.value(),
                                                                 *found.value(), partial);
