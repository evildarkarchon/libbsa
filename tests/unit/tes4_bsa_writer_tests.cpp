...
TEST_CASE("TES4 BSA writer raw output reopens for every target profile",
    ...
    for (const auto target : targets) {
        ...
        REQUIRE(writer.add_file(expected_entries[0].first, utf8_string_from_path(model_source))
                    .has_value());
        REQUIRE(writer.add_file(expected_entries[1].first, utf8_string_from_path(diffuse_source))
                    .has_value());
        REQUIRE(
            writer.add_bytes(expected_entries[2].first, expected_entries[2].second).has_value());
        REQUIRE(
            writer.add_bytes(expected_entries[3].first, expected_entries[3].second).has_value());
        REQUIRE(
            writer.add_bytes(expected_entries[4].first, expected_entries[4].second).has_value());
...
TEST_CASE("TES4 BSA writer serializes derived file flags and hash-sorted tables",
          "[unit][tes4_bsa_writer]") {
    constexpr std::uint32_t expected_v103_file_flags =
        0x0001U | 0x0002U | 0x0008U | 0x0010U | 0x0100U;
    ...
    CHECK(actual_folder_hashes == sorted_hashes({"Docs\\Misc", "Interface\\Gamma", "Meshes\\Alpha",
                                                 "Scripts\\Beta", "Textures\\Zeta"}));
    ...
    for (std::uint32_t folder_index = 0; folder_index < folder_count; ++folder_index) {
        ...
        for (std::size_t index = 0; index + 1U < encoded_name_size; ++index) {
            folder_name.push_back(
                static_cast<char>(std::to_integer<unsigned char>(bytes[cursor + index])));
        }
        cursor += encoded_name_size;

        const auto file_count_for_folder =
            read_u32_le_at(bytes, header_size + folder_index * legacy_folder_record_size + 8U);
...
TEST_CASE("TES4 BSA writer rejects duplicate canonical archive paths at write time",
          "[unit][tes4_bsa_writer]") {
...
TEST_CASE("TES4 BSA writer groups mixed-case folder spellings by canonical path",
          "[unit][tes4_bsa_writer]") {
    libbsa::tes4_bsa_writer_options options;
    options.compression_policy = libbsa::archive_compression_policy::all_raw;
    options.overwrite_existing = true;
...
TEST_CASE("TES4 BSA writer reports invalid archive paths as invalid arguments",
          "[unit][tes4_bsa_writer]") {
...
TEST_CASE("TES4 BSA writer gates parseable DDS texture formats by target profile",
    ...
    SECTION(
        "DX9 BC3 DDS payloads remain valid for Skyrim LE and earlier target "
        "profiles") {
        const auto bc3_dds = read_generated_dds("ba2_dx10_bc3_unorm.dds");
        const std::array targets{libbsa::tes4_bsa_target::oblivion,
    ...
    SECTION("DX10 BC7 DDS payloads require the Skyrim SE BSA target") {
        ...
        auto disk_written =
            disk_writer.write_to(output_path("fallout3-disk-bc7-texture.bsa").string());
...
TEST_CASE(
    "TES4 BSA writer compresses inherited entries and preserves raw "
    "overrides under all-compressed policy",
    "[unit][tes4_bsa_writer]") {
    const std::array targets{libbsa::tes4_bsa_target::oblivion, libbsa::tes4_bsa_target::fallout3,
                             libbsa::tes4_bsa_target::skyrim_se};

    const auto inherited_bytes = bytes_from_text("compress me according to the target profile");
    const auto raw_override_bytes =
        bytes_from_text("keep this entry raw even though the archive default is compressed");
    ...
    for (const auto target : targets) {
        ...
        REQUIRE(writer
                    .add_bytes("Meshes/RawOverride.nif", raw_override_bytes,
...
TEST_CASE(
    "TES4 BSA writer records compressed override metadata while archive "
    "default remains raw",
    "[unit][tes4_bsa_writer]") {
    constexpr std::uint32_t archive_compress_by_default = 0x0004U;
    const std::array targets{libbsa::tes4_bsa_target::oblivion, libbsa::tes4_bsa_target::fallout3,
    ...
    const auto compressed_override_bytes =
        bytes_from_text("compressed per-entry override from all-raw archive");
    ...
    for (const auto target : targets) {
        ...
        REQUIRE(writer
                    .add_bytes("Textures/CompressedOverride.dds", compressed_override_bytes,
        ...
        const auto& compressed_override =
            require_entry(opened.value(), "Textures/CompressedOverride.dds");
        CHECK(compressed_override.compression == compressed_entry_method(target));
        require_extracted_bytes(opened.value(), "Textures/CompressedOverride.dds",
                                compressed_override_bytes);
    }
}

TEST_CASE(
    "TES4 BSA writer keeps compressed disk entries byte-stable with "
    "memory entries",
    "[unit][tes4_bsa_writer][writer-source-io]") {
    const auto source_bytes =
        bytes_from_text("compressed disk source bytes should match memory output");
    ...
    libbsa::tes4_bsa_writer disk_writer{libbsa::tes4_bsa_target::fallout3, options};
    REQUIRE(disk_writer
                .add_file("Meshes/CompressedDisk.nif", source.string(),
                          libbsa::entry_compression_policy::compressed)
                .has_value());
    ...
    REQUIRE(memory_writer
                .add_bytes("Meshes/CompressedDisk.nif", source_bytes,
                           libbsa::entry_compression_policy::compressed)
                .has_value());
...
TEST_CASE("TES4 BSA writer target-default compression round-trips inherited entries",
          "[unit][tes4_bsa_writer]") {
    constexpr std::uint32_t archive_compress_by_default = 0x0004U;
    const std::array targets{libbsa::tes4_bsa_target::oblivion, libbsa::tes4_bsa_target::fallout3,
                             libbsa::tes4_bsa_target::skyrim_se};
    const auto inherited_bytes =
        bytes_from_text("target default inherited bytes must survive extraction");
...
TEST_CASE(
    "TES4 BSA writer overwrites existing archives when "
...
TEST_CASE(
    "TES4 BSA writer leaves embedded names absent by default and when "
    "explicitly disabled",
    "[unit][tes4_bsa_writer]") {
...
TEST_CASE(
    "TES4 BSA writer emits opt-in embedded name prefixes for v104 and "
    "v105 targets",
    "[unit][tes4_bsa_writer]") {
    const std::array cases{
        std::pair{libbsa::tes4_bsa_target::fallout3, std::string{"Meshes/Embedded/Model.nif"}},
        std::pair{libbsa::tes4_bsa_target::skyrim_se, std::string{"Scripts/Embedded/Quest.pex"}}};
    const auto source_bytes =
        bytes_from_text("consumer visible bytes must not include the embedded name");
    ...
    for (const auto& [target, archive_path] : cases) {
        ...
        REQUIRE(static_cast<std::size_t>(entry.payload_offset) + expected_prefix_size <=
                archive_bytes.size());
        CHECK(std::to_integer<unsigned char>(archive_bytes[entry.payload_offset]) ==
              expected_embedded_name.length());
        for (std::size_t index = 0; index < expected_embedded_name.length(); ++index) {
            CHECK(static_cast<char>(std::to_integer<unsigned char>(
                      archive_bytes[entry.payload_offset + 1U + index])) ==
...
TEST_CASE(
    "TES4 BSA writer ignores embedded name option for v103 target "
    "compatibility",
    "[unit][tes4_bsa_writer]") {
...
TEST_CASE(
    "TES4 BSA writer leaves duplicate stored bytes at distinct offsets "
    "by default",
    "[unit][tes4_bsa_writer]") {
...
TEST_CASE("TES4 BSA writer shares offsets for opt-in identical final stored bytes",
          "[unit][tes4_bsa_writer]") {
    const auto source_bytes =
        bytes_from_text("identical source bytes stored twice with opt-in dedupe");
    libbsa::tes4_bsa_writer_options options;
    options.compression_policy = libbsa::archive_compression_policy::all_raw;
...
TEST_CASE("TES4 BSA writer dedupes matching disk-backed raw payloads",
          "[unit][tes4_bsa_writer][writer-source-io]") {
    const auto source_bytes =
        bytes_from_text("identical disk-backed bytes dedupe without full duplicate buffers");
...
TEST_CASE(
    ...
    "[unit][tes4_bsa_writer]") {
    const auto source_bytes =
        bytes_from_text("same source but raw and compressed stored bytes differ");
    libbsa::tes4_bsa_writer_options options;
    options.compression_policy = libbsa::archive_compression_policy::all_raw;
    options.deduplicate_payloads = true;
    options.overwrite_existing = true;
    ...
    REQUIRE(
        writer
            .add_bytes("Meshes/Dedupe/Raw.nif", source_bytes, libbsa::entry_compression_policy::raw)
            .has_value());
    REQUIRE(writer
                .add_bytes("Meshes/Dedupe/Compressed.nif", source_bytes,
                           libbsa::entry_compression_policy::compressed)
                .has_value());
...
TEST_CASE(
    "TES4 BSA writer does not dedupe matching source bytes with "
    "different embedded-name prefixes",
    "[unit][tes4_bsa_writer]") {
    const auto source_bytes =
        bytes_from_text("same source but embedded prefixes include file names");
    libbsa::tes4_bsa_writer_options options;
    options.compression_policy = libbsa::archive_compression_policy::all_raw;
    options.embed_file_names = true;
    options.deduplicate_payloads = true;
