...
std::vector<parsed_entry> parse_tes3_archive(const std::vector<std::byte>& bytes) {
    ...
    for (std::uint32_t index = 0; index < file_count; ++index) {
        ...
        const auto name_offset =
            read_u32_le_at(bytes, name_offsets_start + (index * tes3_name_offset_size));
...
const source_entry& source_for(const std::vector<source_entry>& sources,
                               const parsed_entry& entry) {
    const auto found = std::find_if(
        sources.begin(), sources.end(),
        [&](const source_entry& source) { return source.canonical_path == entry.canonical_path; });
...
std::string manifest_text(const std::vector<std::byte>& archive_bytes,
    ...
    const auto data_section_start =
        tes3_header_size + read_u32_le_at(archive_bytes, 4U) + (file_count * 8U);
    std::ostringstream out;
    out << std::hex << std::setfill('0');
    out << "{\n";
    out << "  \"variant\": \"tes3\",\n";
    ...
    out << "    \"source\": \"synthetic repository-owned bytes; no game or "
           "TES5Edit bytes copied\"\n";
    out << "  },\n";
    out << "  \"entries\": [\n";
...
/// Generates a canonical TES3 writer-output archive and manifest from public
/// writer APIs only.
int main(int argc, char** argv) {
    try {
        ...
        const auto work_dir =
            std::filesystem::temp_directory_path() / "libbsa_tes3_writer_fixture_sources";
        std::filesystem::create_directories(work_dir);

        const source_entry disk_source{
            .source_kind = "disk",
        ...
        const source_entry memory_source{
            .source_kind = "memory",
        ...
        if (auto added = writer.add_file(disk_source.original_path, disk_host_path.string());
            !added.has_value()) {
            throw std::runtime_error("failed to add disk source to TES3 writer fixture");
        }
        if (auto added =
                writer.add_bytes(memory_source.original_path, memory_source.expected_bytes);
            !added.has_value()) {
