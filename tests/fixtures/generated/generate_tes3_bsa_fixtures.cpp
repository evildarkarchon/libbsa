...
std::uint32_t hash_low32(std::uint64_t value) noexcept {
    return static_cast<std::uint32_t>(value & 0xFFFF'FFFFULL);
}

std::uint32_t hash_high32(std::uint64_t value) noexcept {
...
std::vector<entry_spec> windows_unsafe_name_entries() {
    std::vector<entry_spec> entries{
        ...
        {.path = "textures/file.txt:stream",
         .payload = bytes_from_string("ADS-style stream payload\n")},
    };
    for (auto& entry : entries) {
        entry.canonical_path = canonicalize(entry.path);
        entry.archive_hash = libbsa::detail::hash_tes3(entry.path);
...
std::vector<std::byte> build_tes3_archive(std::vector<entry_spec>& entries) {
    ...
    const std::uint32_t hash_table_start =
        tes3_header_size + records_size + name_offsets_size + name_table_size;
    const std::uint32_t data_section_start =
        hash_table_start + checked_u32(entries.size() * 8U, "TES3 hash table");
...
std::string success_manifest(const std::vector<entry_spec>& entries) {
    ...
    std::sort(canonical_sorted.begin(), canonical_sorted.end(),
              [](const entry_spec& lhs, const entry_spec& rhs) {
    ...
    for (std::size_t index = 0; index < canonical_sorted.size(); ++index) {
        ...
        out << "      \"hash_low32\": \"0x" << std::hex << std::setw(8)
            << hash_low32(entry.archive_hash) << "\",\n";
        out << "      \"hash_high32\": \"0x" << std::hex << std::setw(8)
            << hash_high32(entry.archive_hash) << "\",\n";
...
void generate_success(const std::filesystem::path& output_dir) {
    ...
    write_text(output_dir / "tes3_windows_unsafe_names_manifest.json",
               success_manifest(windows_unsafe_entries));
}

void generate_malformed(const std::filesystem::path& output_dir) {
    auto entries = success_entries();
    const auto valid = build_tes3_archive(entries);
    write_file(output_dir / "tes3_truncated_header.bsa",
               std::span<const std::byte>{valid.data(), 6U});
    ...
    const std::size_t hash_table_start =
        entries.empty() ? 0U
                        : entries.front().payload_offset - entries.front().raw_tes3_data_offset -
                              entries.size() * 8U;
...
/// Generates deterministic TES3 BSA fixtures and manifests from synthetic
/// repository-owned bytes.
