...
std::string manifest_for(const archive_spec& archive) {
  ...
  for (std::size_t index = 0; index < archive.entries.size(); ++index) {
    ...
    out << "      \"lookup_variants\": [\"" << json_escape(entry.path) << "\", \"" << json_escape(entry.original_path)
    ...
    out << "      \"stored_size\": " << (entry.compression == libbsa::detail::compression_method::none ? entry.expected_bytes.size() : entry.stored_size) << ",\n";
...
void generate_malformed(const std::filesystem::path& output_dir) {
  ...
  archive_spec duplicate = {.stem = "ba2_duplicate_canonical_path",
                            .variant = "malformed_duplicate_canonical_path",
                            .entries = {{.original_path = "Meshes/Dupe/Same.txt", .expected_bytes = bytes_from_string("first duplicate\n")},
                                        {.original_path = "meshes\\dupe\\same.TXT", .expected_bytes = bytes_from_string("second duplicate\n")}}};
  ...
  write_file(output_dir / "ba2_unsupported_v3_compression_method.ba2", build_archive(unsupported_method));
