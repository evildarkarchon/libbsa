...
void prepare_payload(entry_spec& entry) {
    ...
    if (entry.has_embedded_name) {
        // FO3/SSE embedded names are a length-prefixed filename before the consumer
        // payload.
        entry.embedded_name_prefix_size =
            checked_u32(entry.embedded_name.size() + 1, "embedded name prefix");
    ...
    if (entry.compression == libbsa::detail::compression_method::none) {
        ...
        if (!compressed) {
            throw std::runtime_error("failed to compress fixture payload: " +
...
void write_archive(archive_spec& archive, const std::filesystem::path& output_dir) {
    ...
    const auto folder_block_size =
        checked_u32(archive.folder.size() + 2 +
    ...
    const std::uint32_t metadata_size = libbsa::formats::bsa::tes4_bsa_header_size +
                                        folder_record_size + folder_block_size + file_names_length;
    // TES5Edit stores each folder record offset with the file-name block
    // contribution folded in.
    archive.folder_offset =
        libbsa::formats::bsa::tes4_bsa_header_size + folder_record_size + file_names_length;
...
std::string manifest_for(const archive_spec& archive) {
    ...
    for (std::size_t index = 0; index < archive.entries.size(); ++index) {
        ...
        out << "        \"fnv1a32\": \"0x" << std::hex << std::setw(8)
            << fnv1a32(entry.expected_bytes) << "\"\n";
        out << "      }\n";
...
void generate_malformed(const std::filesystem::path& output_dir) {
    ...
    // Leave the 4-byte exact-size prefix intact so later extraction exercises
    // codec failure.
    ...
    overwrite_u32(size_mismatch_bytes, size_mismatch.entries[1].offset,
                  size_mismatch.entries[1].raw_size + 3U);
...
/// Generates deterministic TES4-family BSA fixtures and JSON manifests for
/// default CI.
