...
namespace
{
  ...
  std::size_t dds_mip_size(std::uint32_t format, std::uint32_t width, std::uint32_t height)
  {
    ...
    {
      return static_cast<std::size_t>(std::max(1U, (width + 3U) / 4U)) * std::max(1U, (height + 3U) / 4U) * block_bytes;
    }
  ...
  std::vector<source_dds_spec> source_dds_valid_specs()
  {
    return {{"bc1_unorm", "ba2_dx10_bc1_unorm.dds", 71U, "BC1_UNORM", 16U, 16U, 1U, 1U, false,
            ...
            {"bc1_unorm_srgb", "ba2_dx10_bc1_unorm_srgb.dds", 72U, "BC1_UNORM_SRGB", 16U, 16U, 1U, 1U, false,
             "textures/formats/bc1_unorm_srgb.dds", "format"},
            ...
            {"r8g8b8a8_snorm", "ba2_dx10_r8g8b8a8_snorm.dds", 31U, "R8G8B8A8_SNORM", 8U, 8U, 1U, 1U, false,
             "textures/formats/r8g8b8a8_snorm.dds", "format"},
            {"multi_mip_bc7_unorm", "ba2_dx10_multi_mip_bc7_unorm.dds", 98U, "BC7_UNORM", 128U, 128U, 5U, 1U, false,
             "textures/structural/multi_mip_bc7_unorm.dds", "multi_mip"},
  ...
  void write_source_case_json(std::ostringstream &out, const source_dds_spec &spec, std::string_view indent)
  {
    out << indent << "{\"id\": \"" << json_escape(spec.id) << "\", \"file\": \"" << json_escape(spec.file)
        << "\", \"format_id\": " << spec.format_id << ", \"format_name\": \"" << json_escape(spec.format_name)
        << "\", \"width\": " << spec.width << ", \"height\": " << spec.height << ", \"mip_count\": " << spec.mip_count
        << ", \"array_size\": " << spec.array_size << ", \"is_cubemap\": " << (spec.is_cubemap ? "true" : "false")
        << ", \"archive_path\": \"" << json_escape(spec.archive_path) << "\", \"structural_case\": \""
  ...
  std::string writer_sources_manifest(const std::vector<source_dds_spec> &specs)
  {
    ...
    out << "  \"provenance\": {\"generator\": \"tests/fixtures/generated/generate_ba2_dx10_fixtures.cpp\", "
           "\"source\": \"repository-owned synthetic DDS bytes generated for libbsa tests; no game or TES5Edit bytes copied\"},\n";
    out << "  \"valid_cases\": [\n";
    ...
    {
      const auto found = std::find_if(specs.begin(), specs.end(), [&](const source_dds_spec &spec)
                                      { return spec.id == key; });
      if (found == specs.end())
      {
  ...
  std::string extension_fourcc(std::string canonical_path)
  {
    ...
    std::string ext = (dot == std::string::npos || (slash != std::string::npos && dot < slash)) ? std::string{} : canonical_path.substr(dot + 1U);
    ext.resize(4U, '\0');
  ...
  void prepare_texture(texture_spec &texture)
  {
    ...
    for (auto &chunk : texture.chunks)
    {
      ...
      if (!compressed)
      {
        throw std::runtime_error("failed to compress BA2 DX10 fixture chunk: " + compressed.error().message);
      }
  ...
  std::uint32_t name_table_size(const archive_spec &archive)
  {
    ...
    for (const auto &texture : archive.textures)
    {
      size += checked_u32(texture.original_path.size() + 2U, "BA2 DX10 FileTableOffset name table");
    }
    return size;
  }
            ...
            .textures = {{.original_path = "Textures/Generated/Fo4Raw.dds",
                         ...
                         {.original_path = "Textures/Generated/Fo4Cube.dds",
                          ...
                          .chunks = {{.decoded_payload = repeated_bytes(0x60, 4), .segment = {.face_index = 0, .source_chunk_index = 0}},
                                     ...
                                     {.decoded_payload = repeated_bytes(0x80, 4), .segment = {.face_index = 2, .source_chunk_index = 2}},
                                     {.decoded_payload = repeated_bytes(0x90, 4), .segment = {.face_index = 3, .source_chunk_index = 3}},
                                     {.decoded_payload = repeated_bytes(0xA0, 4), .segment = {.face_index = 4, .source_chunk_index = 4}},
                                     {.decoded_payload = repeated_bytes(0xB0, 4), .segment = {.face_index = 5, .source_chunk_index = 5}}}}}};
  }
  ...
  std::string manifest_for(const archive_spec &archive)
  {
    ...
    out << "  \"requirements\": [\"DDS-01\",\"DDS-02\",\"DDS-03\",\"DDS-04\",\"DDS-05\",\"DDS-06\",\"DDS-07\"],\n";
    out << "  \"provenance\": {\n";
    out << "    \"generator\": \"tests/fixtures/generated/generate_ba2_dx10_fixtures.cpp\",\n";
    out << "    \"source\": \"synthetic texture bytes generated for libbsa tests; no game or TES5Edit bytes copied\"\n";
    out << "  },\n";
    out << "  \"logical_segment_rule\": \"logical_texture_segment identity is derived from BA2 format-defined chunk order plus texture metadata; success fixtures do not shuffle cubemap or array chunks out of BA2 order\",\n";
    out << "  \"entries\": [\n";
    for (std::size_t index = 0; index < archive.textures.size(); ++index)
    {
      ...
      out << "      \"expected_public_texture_metadata\": {\"width\": " << texture.width << ", \"height\": " << texture.height
          << ", \"mip_count\": " << static_cast<unsigned int>(texture.num_mips) << ", \"dxgi_format\": "
          << static_cast<unsigned int>(texture.dxgi_format) << ", \"array_size\": " << texture.array_size
          << ", \"is_cubemap\": " << (texture.is_cubemap ? "true" : "false") << "},\n";
      out << "      \"expected_directxtex_metadata\": {\"width\": " << texture.width << ", \"height\": " << texture.height
          << ", \"mipLevels\": " << static_cast<unsigned int>(texture.num_mips) << ", \"format\": "
          << static_cast<unsigned int>(texture.dxgi_format) << ", \"arraySize\": " << (texture.is_cubemap ? texture.array_size * 6U : texture.array_size)
          << ", \"isCubemap\": " << (texture.is_cubemap ? "true" : "false") << "},\n";
      out << "      \"expected_dds_payload\": {\"bytes_hex\": \"" << to_hex(decoded) << "\", \"sha256_placeholder\": \"" << to_hex(decoded) << "\"},\n";
      out << "      \"chunks\": [\n";
      for (std::size_t chunk_index = 0; chunk_index < texture.chunks.size(); ++chunk_index)
      {
        const auto &chunk = texture.chunks[chunk_index];
        out << "        {\"offset\": " << chunk.payload_offset << ", \"packed_size\": " << chunk.packed_size
            << ", \"raw_size\": " << chunk.raw_size << ", \"start_mip\": " << chunk.start_mip << ", \"end_mip\": "
            << chunk.end_mip << ", \"sentinel\": \"0xBAADF00D\", \"compression_route\": \"" << compression_route_name(chunk.compression)
            << "\", \"decoded_bytes_hex\": \"" << to_hex(chunk.decoded_payload) << "\", \"logical_texture_segment\": {\"array_index\": "
            << chunk.segment.array_index << ", \"face_index\": " << chunk.segment.face_index << ", \"start_mip\": "
            << chunk.segment.start_mip << ", \"end_mip\": " << chunk.segment.end_mip << ", \"source_chunk_index\": "
            << chunk.segment.source_chunk_index << "}}" << (chunk_index + 1U == texture.chunks.size() ? "\n" : ",\n");
      }
      out << "      ]\n";
  ...
  void generate_writer_sources(const std::filesystem::path &source_dir)
  {
    ...
    write_file(source_dir / "ba2_dx10_malformed_truncated.dds", bytes({0x44, 0x44, 0x53, 0x20, 0x7C}));
  ...
  void generate_malformed(const std::filesystem::path &output_dir)
  {
    ...
    overwrite_u64(invalid_payload_span, static_cast<std::size_t>(first_second_chunk_offset), 0xFFFF'FFFF'FFFF'FFF0ULL);
    write_file(output_dir / "ba2_dx10_invalid_payload_span.ba2", invalid_payload_span);

    auto inconsistent_sizes = valid_fo4;
    overwrite_u32(inconsistent_sizes, static_cast<std::size_t>(first_second_chunk_offset + 12ULL), 0U);
