...
- `archive_hash`, `hash_low32`, and `hash_high32` — stored TES3 hash values generated by
  `libbsa::detail::hash_tes3`. A hash record serializes the two halves as consecutive little-endian
  `u32` values, `hash_high32` (the first-half byte sum) first and `hash_low32` second, so reading the
  eight bytes as one `u64` yields the halves transposed. Records are ordered by ascending
  `archive_hash`, which compares the stored words in the order they appear on disk.
