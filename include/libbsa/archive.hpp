...
/// BA2-specific archive-level metadata exposed when `archive_metadata::type` is
/// `archive_type::ba2`.
///
/// Starfield BA2 revisions append raw header fields after the common
/// `BTDX`/subtype header. These fields are intentionally version-gated
/// optionals so Fallout 4 archives can distinguish "field absent" from a
/// Starfield field that is present with a zero value.
struct ba2_archive_metadata {
    /// Raw Starfield v2+ header field conventionally named `Unknown1` in current
    /// references.
    std::optional<std::uint32_t> starfield_unknown1;

    /// Raw Starfield v2+ header field conventionally named `Unknown2` in current
    /// references.
    std::optional<std::uint32_t> starfield_unknown2;

    /// Raw Starfield v3 compression method field; method `3` selects raw LZ4
    /// block payloads.
    std::optional<std::uint32_t> compression_method;
};

/// Per-chunk BA2 texture payload metadata exposed for dependency-light
/// inspection.
///
/// Offsets are archive-absolute and sizes describe the stored chunk bytes plus
/// the decoded payload bytes. The compression route is a libbsa-owned value so
/// callers never depend on private codec or texture-analysis libraries.
struct texture_chunk_metadata {
    /// Archive-absolute byte offset of this chunk's stored payload.
    std::uint64_t payload_offset;

    /// Stored byte count in the archive; zero-sized stored metadata is
    /// represented as raw chunks by parsers.
...
/// Dependency-light BA2 texture metadata attached only to texture archive
/// entries.
///
/// The format field stores the raw numeric graphics format identifier used by
/// DDS DXT10 metadata, preserving the value without exposing platform or
/// analyzer types. For texture entries, `entry_metadata::raw_size` is the
/// consumer-visible reconstructed DDS size including its header; archive
/// payload sizes remain on the per-chunk metadata below.
struct texture_metadata {
    ...
    /// Raw BA2 texture record byte with currently unknown Bethesda-specific
    /// semantics.
    ...
    /// Chunk layout and compression metadata in archive order unless a later
    /// parser documents otherwise.
...
struct bulk_extract_request {
    /// Archive path to extract, using the same lookup rules as
...
class LIBBSA_API bulk_extract_sink_factory {
   public:
    ...
    /// Creates the sink that will receive the payload for the unique request
    /// `path`.
    ///
...
class archive_reader {
   public:
    ...
    [[nodiscard]] LIBBSA_API result<std::optional<entry_metadata>> find(
    ...
    [[nodiscard]] LIBBSA_API result<std::vector<std::byte>> extract_bytes(
        std::string_view path) const;
