...
| Public/package-consumer API proof | Proven | `tests/package-consumer/main.cpp`; `docs/integration-examples.md`; `tests/unit/public_include_boundary_tests.cpp`; `tests/unit/export_surface_policy_tests.cpp` | Package-consumer source compiles representative `ba2_dx10_writer` target/options usage and the documented example compiles through `<libbsa/libbsa.hpp>`. The installed `package_consumer_smoke` runtime generates a tiny legal BC1 DXT10 DDS input inline, creates a writer-produced BA2 DX10 archive, opens it through `archive_reader`, validates it through `validate_archive`, and extracts the reconstructed DDS through sink, `extract_bytes`, and bulk extraction paths. |
