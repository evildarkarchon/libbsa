...
- Public/package-consumer proof: `tests/package-consumer/main.cpp`, `tests/package-consumer/smoke.cmake`, `tests/package-consumer/verify-runtime-dll-copy.cmake`, plus the `package_consumer_smoke` and `package_consumer_runtime_dll_copy` CTest gates referenced by `tests/unit/target_format_policy_tests.cpp`.
...
| Validation API behavior | Partial | `include/libbsa/validation.hpp`; `tests/unit/validation_api_tests.cpp`; `tests/unit/compatibility_warning_tests.cpp` | The validation API has direct generated-fixture success proof for `tes4_v103.bsa`, direct writer-produced TES4 proof, malformed BSA proof, and compatibility-warning proof. Direct success validation rows for every v104/v105 generated fixture are not currently enumerated; see `COV-GAP-001`. |
...
| Public/package-consumer API proof | Proven | `tests/package-consumer/main.cpp`; `docs/integration-examples.md`; `tests/unit/public_include_boundary_tests.cpp`; `tests/unit/export_surface_policy_tests.cpp` | Package-consumer source compiles representative `tes4_bsa_writer`, `archive_reader`, validation, extraction, and error-handling usage through `<libbsa/libbsa.hpp>`. |
...
| Validation API behavior | Partial | `include/libbsa/validation.hpp`; `tests/unit/validation_api_tests.cpp`; `tests/unit/compatibility_warning_tests.cpp` | The validation API directly accepts generated Fallout 4 BA2 GNRL and representative writer-produced BA2 GNRL archives, and malformed matrix rows cover BA2 GNRL failures. Direct validation success rows for Starfield v2/v3 generated fixtures are not enumerated; see `COV-GAP-001`. |
...
| Public/package-consumer API proof | Proven | `tests/package-consumer/main.cpp`; `docs/integration-examples.md`; `tests/unit/public_include_boundary_tests.cpp`; `tests/unit/export_surface_policy_tests.cpp` | Package-consumer source compiles representative `ba2_gnrl_writer`, target enum, compression policy, reader, validation, and error-handling usage through the installed public header. |
...
| Axis | Status | Default evidence | Notes |
|---|---|---|---|
| Reader/open/list metadata | Proven | `include/libbsa/archive.hpp`; `tests/unit/ba2_dx10_metadata_tests.cpp`; `tests/unit/ba2_dx10_parser_tests.cpp`; `tests/fixtures/generated/archives/ba2_dx10_fo4_manifest.json`; `tests/fixtures/generated/archives/ba2_dx10_sfv3_manifest.json` | Reader tests prove Fallout 4 and Starfield v3 texture archive opening, dependency-light texture metadata, chunk metadata, canonical lowercase paths, original spelling, sparse opening, chunk order validation, and hash checks. |
...
| Writer | Proven | `include/libbsa/writer.hpp`; `tests/unit/ba2_dx10_writer_tests.cpp`; `tests/fixtures/generated/source/ba2_dx10_writer_sources_manifest.json` | Public writer target profiles are `ba2_dx10_target::fallout4` and `starfield_v3`; tests cover DDS source manifest policy, malformed/unsupported DDS rejection, source snapshot lifecycle, target format restrictions, FO4 deflate reopening, Starfield method 3 raw-LZ4 reopening, Starfield method 0 deflate reopening, multi-mip arrays, cubemaps, dedupe, and publication behavior. |
...
| Malformed handling | Proven | `tests/unit/ba2_dx10_parser_tests.cpp`; `tests/unit/ba2_dx10_malformed_tests.cpp`; `tests/fixtures/generated/archives/ba2_dx10_malformed_manifest.json`; `tests/fixtures/generated/compatibility_matrix.json`; `tests/unit/validation_api_tests.cpp` | Default proof covers truncated records, invalid chunk spans, decoded-size mismatches, duplicate mip/face layouts, unsupported compression, oversized filename offsets, aggregate chunk limits, hash mismatches, and validation diagnostics. |
| Validation API behavior | Partial | `include/libbsa/validation.hpp`; `tests/unit/validation_api_tests.cpp` | The validation API directly accepts generated Fallout 4 BA2 DX10 and representative writer-produced BA2 DX10 archives, and malformed matrix rows cover BA2 DX10 failures. Direct validation success rows for Starfield v3 DX10 generated fixtures and both Starfield compression methods are not enumerated; see `COV-GAP-001`. |
...
| Public/package-consumer API proof | Proven | `tests/package-consumer/main.cpp`; `docs/integration-examples.md`; `tests/unit/public_include_boundary_tests.cpp`; `tests/unit/export_surface_policy_tests.cpp` | Package-consumer source compiles representative `ba2_dx10_writer` target/options usage and the documented example compiles through `<libbsa/libbsa.hpp>`. Runtime texture archive behavior is proven by unit fixture tests rather than package-consumer runtime fixtures; see `COV-GAP-003`. |
| Docs/support-claim proof | Proven | `docs/target-format-guide.md`; `docs/api-mainpage.md`; `tests/unit/target_format_policy_tests.cpp`; `tests/unit/docs_policy_tests.cpp` | Target-format policy tests require Fallout 4 DX10, Starfield v3 DX10, raw LZ4 block, deflate routes, writer target policies, and public documentation boundaries. |
...
|---:|---|---|---|---|---|
| 1 | `COV-GAP-001` | Medium | Validation API behavior for variant-specific success rows: TES4 v104/v105, BA2 GNRL Starfield v2/v3, and BA2 DX10 Starfield v3 compression-method routes. | `tests/unit/validation_api_tests.cpp` directly covers TES3, TES4 v103, Fallout 4 BA2 GNRL, Fallout 4 BA2 DX10, representative writer-produced archives, and malformed matrix rows; family reader/writer tests prove the remaining variants indirectly. | Add a compact validation-success parameter matrix if the validation/error-behavior audit needs direct per-variant validation proof. |
| 2 | `COV-GAP-002` | Low / Deferred | Full BSArchPro or real game-corpus compatibility comparisons for all families. | `tests/unit/local_game_fixture_tests.cpp`, `tests/fixtures/README.md`, and `docs/compatibility-evidence.md` document opt-in local checks, but default evidence intentionally avoids copyrighted archive bytes. | Future compatibility-hardening campaign using local manifests; keep default CI independent of local/copyrighted inputs. |
| 3 | `COV-GAP-003` | Low | Package-consumer runtime proof for creating/opening every archive family from an installed package. | `tests/package-consumer/main.cpp` compile/links representative public APIs and smoke-checks error behavior; actual archive behavior is covered by always-on unit fixture tests. | S05 integrated confidence/package-release polish can add installed-package fixture runtime tests if package-level behavior, not library behavior, becomes the risk. |
| 4 | `COV-GAP-004` | Low / Deferred | Family-specific compatibility warning taxonomy for TES3 BSA and BA2 DX10. | `include/libbsa/validation.hpp`, `tests/unit/compatibility_warning_tests.cpp`, and `docs/compatibility-evidence.md` prove every current public warning code; no TES3 or BA2 DX10-specific warning is currently claimed. | Add warning codes only when a known target interoperability risk is identified and can be proven with default generated or writer-output evidence. |
