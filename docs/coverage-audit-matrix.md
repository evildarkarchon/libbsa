...
| Malformed handling | Proven | `tests/unit/ba2_gnrl_reader_tests.cpp`; `tests/fixtures/generated/archives/ba2_gnrl_malformed_manifest.json`; `tests/fixtures/generated/compatibility_matrix.json`; `tests/unit/validation_api_tests.cpp` | Default proof covers truncated name tables, invalid payload spans, unsupported Starfield v3 compression methods, exact-size mismatches, duplicate paths, oversized tables, and validation diagnostics. Stored record hash disagreement is deliberately not in this list: it is a compatibility warning, not a malformed-archive condition. |
...
| Compatibility warnings | Proven | `include/libbsa/validation.hpp`; `tests/unit/compatibility_warning_tests.cpp`; `tests/unit/ba2_gnrl_reader_tests.cpp`; `docs/compatibility-evidence.md` | Current BA2 warning proof covers caller-supplied target-family mismatch and BA2 record identity mismatch through generated writer-output validation. No further GNRL-specific warning beyond the current catalog is claimed. |
...
| Malformed handling | Proven | `tests/unit/ba2_dx10_reader_tests.cpp`; `tests/unit/ba2_dx10_malformed_tests.cpp`; `tests/fixtures/generated/archives/ba2_dx10_malformed_manifest.json`; `tests/fixtures/generated/compatibility_matrix.json`; `tests/unit/validation_api_tests.cpp` | Default proof covers truncated records, invalid chunk spans, decoded-size mismatches, duplicate mip/face layouts, unsupported compression, oversized filename offsets, aggregate chunk limits, and validation diagnostics. Stored record hash disagreement is deliberately not in this list: it is a compatibility warning, not a malformed-archive condition. |
...
| Compatibility warnings | Proven | `include/libbsa/validation.hpp`; `tests/unit/compatibility_warning_tests.cpp`; `tests/unit/ba2_dx10_reader_tests.cpp`; `docs/compatibility-evidence.md` | BA2 DX10 shares the record-identity-mismatch warning with GNRL and proves it against generated fixtures. Texture format restrictions remain writer/parser validation behavior rather than compatibility warnings. |
