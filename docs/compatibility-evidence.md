...
- Deliberately not demoted alongside it: the sibling `TotalFolderNameLength` total stays a fatal `format_error`. Issue #45 asked for both to be demoted or dropped, but the two are not the same kind of claim. Surplus *file*-name bytes are inert -- no entry is read from them, so an archive carrying them lists and extracts completely. `TotalFolderNameLength` is load-bearing: `tes4_bsa_metadata_table_size` sizes the whole metadata span from it, so a wrong value places the file-name table somewhere the parser did not read. Demoting it would open no further archive, only trade a precise diagnostic for a confusing downstream one. Every archive in the retail corpus satisfies it once the bzstring length prefix is excluded from the count (commit `e97a0e2f`).
