...
- **Global scope**: `J:/libbsa/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `J:/libbsa/.codex/notes/`)
...
2. Glob `J:/libbsa/.codex/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `J:/libbsa/.codex/notes/` — works in any directory
