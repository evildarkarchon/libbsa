...
- Issue: Several format modules still carry parsing, validation, metadata materialization, and compatibility decisions in large translation units. `src/formats/ba2/ba2_dx10_parser.cpp` has been narrowed by moving fixed-header/record parsing and count-delimited filename-table reading into dedicated helpers, but it still owns DX10 extension/hash validation, chunk span validation, public texture metadata construction, and file-backed orchestration. `src/formats/ba2/ba2_gnrl_parser.cpp`, `src/formats/bsa/tes3_bsa_parser.cpp`, and `src/formats/bsa/tes4_bsa_prepare.cpp` continue to follow the broader pattern.
- Files: `src/formats/ba2/ba2_dx10_parser.cpp`, `src/formats/ba2/ba2_dx10_records.cpp`, `src/formats/ba2/ba2_dx10_names.cpp`, `src/formats/ba2/ba2_gnrl_parser.cpp`, `src/formats/bsa/tes3_bsa_parser.cpp`, `src/formats/bsa/tes4_bsa_prepare.cpp`
- Impact: Small archive-format fixes can accidentally change unrelated behavior such as hash validation, filename normalization, metadata allocation policy, or extraction ordering. Future Starfield/BA2 edge cases are less likely to disturb fixed-header/record and filename-table readers, but metadata materialization and compatibility validation remain coupled.
- Fix approach: Continue splitting by responsibility when modifying these areas: keep fixed-header/record readers, filename-table readers, span validation, and public metadata materialization in separate helpers with focused tests in `tests/unit/ba2_dx10_parser_tests.cpp`, `tests/unit/ba2_gnrl_reader_tests.cpp`, and related malformed fixture tests.
...
**BA2/BSA overlap arithmetic now uses checked central helper:**
- Status: Resolved in this refactor. The repeated local `spans_overlap_u64()` helpers in BA2 DX10, BA2 GNRL, and TES4 BSA parsing were replaced with `detail::spans_overlap_u64()` in `src/detail/parser_primitives.cpp`, which saturates overflowing end offsets instead of adding unchecked `start + length`.
- Files: `src/detail/parser_primitives.hpp`, `src/detail/parser_primitives.cpp`, `src/formats/ba2/ba2_dx10_parser.cpp`, `src/formats/ba2/ba2_gnrl_parser.cpp`, `src/formats/bsa/tes4_bsa_parser.cpp`, `tests/unit/parser_primitives_tests.cpp`
- Follow-up: Keep new span-overlap checks routed through `detail::spans_overlap_u64()` rather than adding parser-local copies.
