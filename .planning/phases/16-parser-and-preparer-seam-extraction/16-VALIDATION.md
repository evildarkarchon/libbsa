...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 16-01-01 | 01 | 1 | REFA-01 | T-16-01 | TES4 table seam preserves table sizing, folder/file-name offsets, name/hash validation, and duplicate canonical path behavior. | unit + fixture/malformed | `ctest --preset windows-msvc-debug-static --output-on-failure -L "parser_preparer_seam"` | Wave 0: add `tests/unit/tes4_bsa_parser_seam_tests.cpp`; existing `tests/unit/tes4_bsa_reader_tests.cpp` remains covered by the affected-suite wave gate. | pending |
| 16-01-02 | 01 | 1 | REFA-01 | T-16-02 | TES4 payload descriptor seam preserves embedded-name prefix sizing, raw-size calculation, compression interpretation, and payload-span-over-metadata rejection. | unit + malformed | `ctest --preset windows-msvc-debug-static --output-on-failure -L "parser_preparer_seam"` | Wave 0: direct seam tests needed; existing public malformed tests remain covered by the affected-suite wave gate. | pending |
| 16-02-01 | 02 | 1 | REFA-02 | T-16-03 | BA2 DX10 snapshot builder preserves source snapshot immutability and target DDS format validation. | unit + writer integration | `ctest --preset windows-msvc-debug-static --output-on-failure -L "parser_preparer_seam"` | Wave 0: add `tests/unit/ba2_dx10_preparer_seam_tests.cpp`; existing `tests/unit/ba2_dx10_writer_tests.cpp` remains covered by the affected-suite wave gate. | pending |
| 16-02-02 | 02 | 1 | REFA-02 | T-16-04 | BA2 DX10 chunk seam preserves streamed snapshot-backed assembly, multi-mip/array/cubemap order, and Fallout 4 vs Starfield v3 compression routing. | unit + writer-stage | `ctest --preset windows-msvc-debug-static --output-on-failure -L "parser_preparer_seam"` | Existing `tests/unit/writer_stage_tests.cpp` covers single/multi/cubemap/truncated; add direct routing/plan-then-assemble seam tests and keep writer-stage in the affected-suite wave gate. | pending |
...
- [ ] No watch-mode flags
- [ ] Feedback latency < 30s for focused task smoke feedback after incremental build
- [ ] `nyquist_compliant: true` remains set while task smoke commands stay narrow and affected/full/ASan commands remain wave or phase gates
