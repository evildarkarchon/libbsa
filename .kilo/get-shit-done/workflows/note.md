...
- **Global scope**: `J:/libbsa/.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `J:/libbsa/.kilo/notes/`)
...
2. Glob `J:/libbsa/.kilo/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `J:/libbsa/.kilo/notes/` — works in any directory
