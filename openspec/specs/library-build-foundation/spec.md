# library-build-foundation Specification

## Purpose
TBD - created by archiving change m1-foundation-tes4-read-support. Update Purpose after archive.
## Requirements
### Requirement: Static and dynamic library project layout
libbsa SHALL provide a C++20 library project layout with public headers under `include/`, implementation sources under `src/`, and automated tests under `tests/`.
...
#### Scenario: Configure and build dynamic library
- **WHEN** the project is configured with the repo shared-library CMake preset
- **THEN** CMake builds a dynamic `libbsa` library target and exposes the `libbsa::libbsa` alias for consumers.
